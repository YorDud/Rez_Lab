...
{
	public partial class Pryam_Metal_Met_Laborant : Form
	{
		public Pryam_Metal_Met_Laborant()
		{
			InitializeComponent();
			//label11.Text = "PdCl2 - 0,4 - 0,5 г/л\r\nSnCl2 - 10 - 18 г/л\r\nHCl -\r\nCu 2+ < 1 г/л\r\nОБЪЕМ ВАННЫ: 224 л";
		...
		private void LoadData()
		{
			...
			{
				connection.Open();
				dataAdapter = new SqlDataAdapter("SELECT * FROM Pryam_Metal_Met ORDER BY Date_Create DESC", connection);
				...
				dataGridView1.Columns["Pr_Met_3_Correction_Mat"].HeaderText = "Корректировка материал";
				dataGridView1.Columns["Pr_Met_3_Correction_Score"].HeaderText = "Корректировка количество";
				dataGridView1.Columns["Pr_Met_3_ProshDM2"].HeaderText = "Прошедшие дм²";
...
dataGridView1.Columns["Сomment"].HeaderText = "Комментарий";
		...
		private void Pryam_Metal_Met_Laborant_Load(object sender, EventArgs e)
		{
			LoadData();
		...
		private void LoadDataDateTimePicker(DateTime date)
		{
			using (SqlConnection connection = new SqlConnection(WC.ConnectionString))
			{
				...
				string query = "SELECT * FROM Pryam_Metal_Met WHERE CAST(Date_Create AS DATE) = @SelectedDate ORDER BY Date_Create DESC";
		...
		private void LoadDataByDateRange(DateTime startDate, DateTime endDate)
		{
			...
			{
				connection.Open();
				string query = "SELECT * FROM Pryam_Metal_Met WHERE CAST(Date_Create AS DATE) BETWEEN @StartDate AND @EndDate ORDER BY Date_Create DESC";
				using (SqlCommand command = new SqlCommand(query, connection))
				{
		...
		private void Add_Row()
		{
			using (SqlConnection connection = new SqlConnection(WC.ConnectionString))
			{
				...
				string sqlInsert = "INSERT INTO Pryam_Metal_Met ([Date_Create],[FIO_Lab]) " +
		...
		private void UpdateRow()
		{
			...
			using (SqlConnection connection = new SqlConnection(WC.ConnectionString))
			{
				...
				string updateQuery = @"
            UPDATE Pryam_Metal_Met
   SET [Pr_Met_3_PdCl2] = @Pr_Met_3_PdCl2,
...
[Pr_Met_3_Cu2] = @Pr_Met_3_Cu2
	...
	WHERE ID = @ID";
		...
		private void NotClickOnTable()
		{
			...
			{
				if (column.Name != "Pr_Met_3_PdCl2" && column.Name != "Pr_Met_3_HCl" && column.Name != "Pr_Met_3_SnCl2" && column.Name != "Pr_Met_3_Cu2")
				{
...
[Pr_Met_3_Cu2] = @Pr_Met_3_Cu2
		...
		private void dataGridView1_CellClick(object sender, DataGridViewCellEventArgs e)
		{
			if (e.RowIndex >= 0 && e.ColumnIndex == 6) // Проверяем, что ячейка выбрана
			{
				...
				using (Form inputForm = new Form())
				{
					...
					{
						Text = "Н - показание порт. колориметра\r\nР - разбавление",
						Top = 40,
						Left = 10,
						Width = 330,
						Height = 100
			...
			else if (e.RowIndex >= 0 && e.ColumnIndex == 5) // Проверяем, что ячейка выбрана
			{
				...
				using (Form inputForm = new Form())
				{
					...
					inputForm.MinimizeBox = false;
					inputForm.StartPosition = FormStartPosition.CenterParent;
					inputForm.Text = "SnCl2";

					Label labelValue1 = new Label()
					{
						//Text = "C(SnCl2) = (10 - (a * c)) * F * 11,28",
						Text = "С(SnCl2) = a × T × 1,901 × 1000 / Vпр ",
						Top = 10,
						Left = 30,
						Width = 330
					};
					...
					Label labelValueA = new Label()
					{
						//Text = "а - колич. 0,1Н раствора тиосульфата, израсход. на титрование, мл\r\nF - поправ. коэф. к 0,1Н раствору йода; [=1]\r\nc - 10/в\r\nв - холостая проба",
						Text = "а - количество 0,1Н раствора йода, израсходованное на титрование, мл\r\nT – титр 0,1Н раствора йода по олову (0,00593 г/мл)\r\nVпр – объем вводимой пробы, мл.",
						Top = 40,
						Left = 10,
						Width = 330,
						Height = 100
					...
					calculateButton.Click += (s, args) =>
					{
						...
						if (double.TryParse(value1.Text, out double v1) && double.TryParse(value2.Text, out double v2))
						{
							double result = Math.Round((v1 * 5.93 * 1.901 / v2), 2); // Пример формулы (сложение)
			...
			else if (e.RowIndex >= 0 && e.ColumnIndex == 4) // Проверяем, что ячейка выбрана
			{
				...
				using (Form inputForm = new Form())
				{
					...
					Label labelValueA = new Label()
					{
						Text = "а - колич. 0,1Н раствора NaOH, израсход. на титрование, мл",
						Top = 40,
						Left = 10,
						Width = 330,
						Height = 100
					...
					calculateButton.Click += (s, args) =>
					{
						...
						{
							double result = Math.Round(v1 * 3.65, 2); // Пример формулы (сложение)
	...
	WHERE ID = @ID";
				...
				using (SqlCommand command = new SqlCommand(updateQuery, connection))
				{
					...
					command.Parameters.AddWithValue("@Date_Lab_Update", Date_Lab_Update);
					command.Parameters.AddWithValue("@FIO_Lab_Update", FIO_Lab_Update);
					command.Parameters.AddWithValue("@Pr_Met_3_PdCl2", Pr_Met_3_PdCl2 ?? DBNull.Value);
					command.Parameters.AddWithValue("@Pr_Met_3_HCl", Pr_Met_3_HCl ?? DBNull.Value);
					command.Parameters.AddWithValue("@Pr_Met_3_SnCl2", Pr_Met_3_SnCl2 ?? DBNull.Value);
					command.Parameters.AddWithValue("@Pr_Met_3_Cu2", Pr_Met_3_Cu2 ?? DBNull.Value);
					command.Parameters.AddWithValue("@ID", id); // ID - это первичный ключ
