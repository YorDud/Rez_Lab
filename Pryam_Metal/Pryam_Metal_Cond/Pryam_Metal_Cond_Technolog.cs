...
{
	public partial class Pryam_Metal_Cond_Technolog : Form
	{
		public Pryam_Metal_Cond_Technolog()
		{
			InitializeComponent();
			//label11.Text = "ПМ 302 - 90-110 мл/л\r\nCu 2+ < 1 г/л\r\nОБЪЕМ ВАННЫ: 234 л";
		...
		private void LoadData()
		{
			...
			{
				connection.Open();
				dataAdapter = new SqlDataAdapter("SELECT * FROM Pryam_Metal_Cond ORDER BY Date_Create DESC", connection);
				...
				//
				//dataGridView1.Columns["Pr_Met_1_PM302"].HeaderText = "ПМ 302";
				dataGridView1.Columns["Pr_Met_1_PM302"].HeaderText = "Щелочность";
				//dataGridView1.Columns["Pr_Met_1_Cu2"].HeaderText = "Cu 2+";
				dataGridView1.Columns["Pr_Met_1_Cu2"].HeaderText = "pH";
...
dataGridView1.Columns["Сomment"].HeaderText = "Комментарий";
				...
				dataGridView1.Columns["Pr_Met_1_Correction_Mat"].DefaultCellStyle.BackColor = Color.PeachPuff;
				dataGridView1.Columns["Pr_Met_1_Correction_Score"].DefaultCellStyle.BackColor = Color.PeachPuff;
		...
		private void Pryam_Metal_Cond_Technolog_Load(object sender, EventArgs e)
		{
			LoadData();
		...
		private void NotClickOnTable()
		{
			...
			{
				if (column.Name != "Pr_Met_1_Correction_Score" && column.Name != "Pr_Met_1_Correction_Mat" && column.Name != "Сomment" && column.Name != "Pr_Met_1_ProshDM2") // предполагается, что "ID" - это столбец, который можно редактировать
				{
		...
		private void LoadDataDateTimePicker(DateTime date)
		{
			using (SqlConnection connection = new SqlConnection(WC.ConnectionString))
			{
				...
				string query = "SELECT * FROM Pryam_Metal_Cond WHERE CAST(Date_Create AS DATE) = @SelectedDate ORDER BY Date_Create DESC";
				using (SqlCommand command = new SqlCommand(query, connection))
				{
		...
		private void LoadDataByDateRange(DateTime startDate, DateTime endDate)
		{
			...
			{
				connection.Open();
				string query = "SELECT * FROM Pryam_Metal_Cond WHERE CAST(Date_Create AS DATE) BETWEEN @StartDate AND @EndDate ORDER BY Date_Create DESC";
				using (SqlCommand command = new SqlCommand(query, connection))
				{
			...
			if (e.RowIndex >= 0 && e.ColumnIndex == 0)
			{
				...
				if (idValue != null)
				{
					if (int.TryParse(idValue.ToString(), out WC.id_click))
					{
						...
						string connectionString = WC.ConnectionString;
						string query = @"UPDATE Pryam_Metal_Cond SET
                                 [Date_tech] = @Date_tech,
                                 [FIO_tech] = @FIO_tech
                                 WHERE ID = @Id";
		...
		private void Add_Row()
		{
			using (SqlConnection connection = new SqlConnection(WC.ConnectionString))
			{
				...
				string sqlInsert = "INSERT INTO Pryam_Metal_Cond ([Date_Create],[FIO_Lab]) " +
		...
		private void UpdateRow()
		{
			...
			using (SqlConnection connection = new SqlConnection(WC.ConnectionString))
			{
				...
				var Pr_Met_1_Correction_Mat = dataGridView1.Rows[rowIndex].Cells["Pr_Met_1_Correction_Mat"].Value;
				var Pr_Met_1_Correction_Score = dataGridView1.Rows[rowIndex].Cells["Pr_Met_1_Correction_Score"].Value;
				//var Comment = dataGridView1.Rows[e.RowIndex].Cells["Сomment"].Value;
				...
				string updateQuery = @"
            UPDATE Pryam_Metal_Cond
   SET [Pr_Met_1_Correction_Mat] = @Pr_Met_1_Correction_Mat
			,[Pr_Met_1_Correction_Score] = @Pr_Met_1_Correction_Score
