...
{
	public partial class Pryam_Metal_PredMet303_Laborant : Form
	{
		public Pryam_Metal_PredMet303_Laborant()
		{
			InitializeComponent();
			////label11.Text = "pH - < 0,6\r\nCu 2+ < 0,5 \r\nОБЪЕМ ВАННЫ: 185 л";
		...
		private void LoadData()
		{
			...
			{
				connection.Open();
				dataAdapter = new SqlDataAdapter("SELECT * FROM Pryam_Metal_PredMet303 ORDER BY Date_Create DESC", connection);
...
dataGridView1.Columns["Сomment"].HeaderText = "Комментарий";
		...
		private void Pryam_Metal_PredMet303_Laborant_Load(object sender, EventArgs e)
		{
			LoadData();
		...
		private void LoadDataDateTimePicker(DateTime date)
		{
			using (SqlConnection connection = new SqlConnection(WC.ConnectionString))
			{
				...
				string query = "SELECT * FROM Pryam_Metal_PredMet303 WHERE CAST(Date_Create AS DATE) = @SelectedDate ORDER BY Date_Create DESC";
		...
		private void LoadDataByDateRange(DateTime startDate, DateTime endDate)
		{
			...
			{
				connection.Open();
				string query = "SELECT * FROM Pryam_Metal_PredMet303 WHERE CAST(Date_Create AS DATE) BETWEEN @StartDate AND @EndDate ORDER BY Date_Create DESC";
				using (SqlCommand command = new SqlCommand(query, connection))
				{
		...
		private void Add_Row()
		{
			using (SqlConnection connection = new SqlConnection(WC.ConnectionString))
			{
				...
				string sqlInsert = "INSERT INTO Pryam_Metal_PredMet303 ([Date_Create],[FIO_Lab]) " +
		...
		private void UpdateRow()
		{
			...
			using (SqlConnection connection = new SqlConnection(WC.ConnectionString))
			{
				...
				string updateQuery = @"
            UPDATE Pryam_Metal_PredMet303
   SET [Pr_Met_2_pH] = @Pr_Met_2_pH,
[Pr_Met_2_Cu2] = @Pr_Met_2_Cu2
	...
	WHERE ID = @ID";
		...
		private void NotClickOnTable()
		{
			...
			{
				if (column.Name != "Pr_Met_2_pH" && column.Name != "Pr_Met_2_Cu2")
				{
...
[Pr_Met_2_Cu2] = @Pr_Met_2_Cu2
	...
	WHERE ID = @ID";
		...
		private void dataGridView1_CellClick(object sender, DataGridViewCellEventArgs e)
		{
			...
			if (e.RowIndex >= 0 && e.ColumnIndex == 4) // Проверяем, что ячейка выбрана
			{
				...
				using (Form inputForm = new Form())
				{
					...
					{
						//Text = "C(Cu2+) = a * 0,0635 * K",
						Text = "С(Cu2+) = a × 3,175 / Vпр",
						Top = 10,
						Left = 30,
						Width = 330
					};
					...
					{
						//Text = "а - количество 0,02Н раствора Трилона Б, израсходованной на титрование, мл\r\nK - поправочный коэф. к 0,02Н раствору Трилона Б(0,003177)",
						Text = "a – количество 0,1Н раствора Трилона Б, израсходованное на титрование, мл;\r\nVпр – объем взятой пробы",
						Top = 40,
						Left = 10,
						Width = 330,
						Height = 100
					...
					calculateButton.Click += (s, args) =>
					{
						...
						{
							//double result = Math.Round(v1 * 0.0635 * v2, 2); // Пример формулы (сложение)
							double result = Math.Round(v1 * 3.175 / v2, 2); // Пример формулы (сложение)
	...
	WHERE ID = @ID";
				...
				using (SqlCommand command = new SqlCommand(updateQuery, connection))
				{
					...
					command.Parameters.AddWithValue("@Date_Lab_Update", Date_Lab_Update);
					command.Parameters.AddWithValue("@FIO_Lab_Update", FIO_Lab_Update);
					command.Parameters.AddWithValue("@Pr_Met_2_Cu2", Pr_Met_2_Cu2 ?? DBNull.Value);
					command.Parameters.AddWithValue("@ID", id); // ID - это первичный ключ
