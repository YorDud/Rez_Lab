...
{
	public partial class Himich_Photorez_Dek_Laborant : Form
	{
		public Himich_Photorez_Dek_Laborant()
		{
			InitializeComponent();
...
dataGridView1.Columns["Сomment"].HeaderText = "Комментарий";
				...
				dataGridView1.Columns["Him_Podg_3_H2SO4"].DefaultCellStyle.BackColor = Color.LightBlue;
		...
		private void Himich_Photorez_Dek_Laborant_Load(object sender, EventArgs e)
		{
			LoadData();
		...
		private void UpdateRow()
		{
			...
			using (SqlConnection connection = new SqlConnection(WC.ConnectionString))
			{
				...
				var Him_Podg_3_H2SO4 = dataGridView1.Rows[rowIndex].Cells["Him_Podg_3_H2SO4"].Value;
   ...
   SET [Him_Podg_3_H2SO4] = @Him_Podg_3_H2SO4, 
[Him_Podg_3_Cu2] = @Him_Podg_3_Cu2
	...
	WHERE ID = @ID";
		...
		private void NotClickOnTable()
		{
			...
			{
				if (column.Name != "Him_Podg_3_H2SO4" && column.Name != "Him_Podg_3_Cu2")
				{
			...
			if (e.RowIndex >= 0 && e.ColumnIndex == 3) // Проверяем, что ячейка выбрана
			{
				...
				using (Form inputForm = new Form())
				{
					...
					calculateButton.Click += (s, args) =>
					{
						...
						{
							double result = Math.Round(v1 * 5.452 * v2, 2); 
			...
			else if (e.RowIndex >= 0 && e.ColumnIndex == 4) // Проверяем, что ячейка выбрана
			{
				...
				using (Form inputForm = new Form())
				{
					...
					calculateButton.Click += (s, args) =>
					{
						...
						{
							double result = Math.Round(v1 * 1.27, 2); // Пример формулы (сложение)
