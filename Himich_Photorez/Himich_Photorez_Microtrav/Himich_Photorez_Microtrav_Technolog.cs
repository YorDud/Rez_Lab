...
{
	public partial class Himich_Photorez_Microtrav_Technolog : Form
	{
		public Himich_Photorez_Microtrav_Technolog()
		{
			InitializeComponent();
		...
		private void LoadData()
		{
			...
			{
				connection.Open();
				dataAdapter = new SqlDataAdapter("SELECT * FROM Himich_Photorez_Microtrav ORDER BY Date_Create DESC", connection);
				...
				//
				dataGridView1.Columns["Him_Podg_2_H2SO4"].HeaderText = "H2SO4";
				dataGridView1.Columns["Him_Podg_2_H2O2"].HeaderText = "H2O2";
				dataGridView1.Columns["Him_Podg_2_Cu2"].HeaderText = "Cu 2+";
				dataGridView1.Columns["Him_Podg_2_podtrav"].HeaderText = "Подтрав";
				dataGridView1.Columns["Him_Podg_2_pH"].HeaderText = "pH";
				dataGridView1.Columns["Him_Podg_2_Correction_Mat"].HeaderText = "Корректировка материал";
				dataGridView1.Columns["Him_Podg_2_Correction_Score"].HeaderText = "Корректировка количество";
				//
...
dataGridView1.Columns["Сomment"].HeaderText = "Комментарий";
				...
				dataGridView1.Columns["Him_Podg_2_Correction_Mat"].DefaultCellStyle.BackColor = Color.PeachPuff;
				dataGridView1.Columns["Him_Podg_2_Correction_Score"].DefaultCellStyle.BackColor = Color.PeachPuff;
		...
		private void Himich_Photorez_Microtrav_Technolog_Load(object sender, EventArgs e)
		{
			LoadData();
		...
		private void NotClickOnTable()
		{
			...
			{
				if (column.Name != "Him_Podg_2_Correction_Score" && column.Name != "Him_Podg_2_Correction_Mat" && column.Name != "Сomment") // предполагается, что "ID" - это столбец, который можно редактировать
				{
		...
		private void dataGridView1_CellEndEdit(object sender, DataGridViewCellEventArgs e)
		{
			...
			using (SqlConnection connection = new SqlConnection(WC.ConnectionString))
			{
				...
				var Him_Podg_2_H2SO4 = dataGridView1.Rows[e.RowIndex].Cells["Him_Podg_2_H2SO4"].Value;
				var Him_Podg_2_H2O2 = dataGridView1.Rows[e.RowIndex].Cells["Him_Podg_2_H2O2"].Value;
				var Him_Podg_2_Cu2 = dataGridView1.Rows[e.RowIndex].Cells["Him_Podg_2_Cu2"].Value;
				...
				var Him_Podg_2_Correction_Mat = dataGridView1.Rows[e.RowIndex].Cells["Him_Podg_2_Correction_Mat"].Value;
				var Him_Podg_2_Correction_Score = dataGridView1.Rows[e.RowIndex].Cells["Him_Podg_2_Correction_Score"].Value;
				...
				string updateQuery = @"
        UPDATE Himich_Photorez_Microtrav SET
            [Date_tech] = @Date_tech,
            [Him_Podg_2_H2SO4] = @Him_Podg_2_H2SO4
      ,[Him_Podg_2_H2O2] = @Him_Podg_2_H2O2
      ,[Him_Podg_2_Cu2] = @Him_Podg_2_Cu2,
[Him_Podg_2_podtrav] = @Him_Podg_2_podtrav,
[Him_Podg_2_pH] = @Him_Podg_2_pH,
[Him_Podg_2_Correction_Mat] = @Him_Podg_2_Correction_Mat,
[Him_Podg_2_Correction_Score] = @Him_Podg_2_Correction_Score
			,[Сomment] = @Comment
            ,FIO_tech = @FIO_tech
        WHERE ID = @Id";
				...
				using (SqlCommand command = new SqlCommand(updateQuery, connection))
				{
					...
					command.Parameters.AddWithValue("@Him_Podg_2_H2SO4", Him_Podg_2_H2SO4);
					command.Parameters.AddWithValue("@Him_Podg_2_H2O2", Him_Podg_2_H2O2);
					command.Parameters.AddWithValue("@Him_Podg_2_Cu2", Him_Podg_2_Cu2);
					...
					command.Parameters.AddWithValue("@Him_Podg_2_Correction_Mat", Him_Podg_2_Correction_Mat);
					command.Parameters.AddWithValue("@Him_Podg_2_Correction_Score", Him_Podg_2_Correction_Score);
		...
		private void LoadDataDateTimePicker(DateTime date)
		{
			using (SqlConnection connection = new SqlConnection(WC.ConnectionString))
			{
				...
				string query = "SELECT * FROM Himich_Photorez_Microtrav WHERE CAST(Date_Create AS DATE) = @SelectedDate ORDER BY Date_Create DESC";
				using (SqlCommand command = new SqlCommand(query, connection))
				{
		...
		private void LoadDataByDateRange(DateTime startDate, DateTime endDate)
		{
			...
			{
				connection.Open();
				string query = "SELECT * FROM Himich_Photorez_Microtrav WHERE CAST(Date_Create AS DATE) BETWEEN @StartDate AND @EndDate ORDER BY Date_Create DESC";
				using (SqlCommand command = new SqlCommand(query, connection))
				{
			...
			if (e.RowIndex >= 0 && e.ColumnIndex == 0)
			{
				...
				if (idValue != null)
				{
					if (int.TryParse(idValue.ToString(), out WC.id_click))
					{
						...
						string connectionString = WC.ConnectionString;
						string query = @"UPDATE Himich_Photorez_Microtrav SET
                                 [Date_tech] = @Date_tech,
                                 [FIO_tech] = @FIO_tech
                                 WHERE ID = @Id";
		...
		private void Add_Row()
		{
			using (SqlConnection connection = new SqlConnection(WC.ConnectionString))
			{
				...
				string sqlInsert = "INSERT INTO Himich_Photorez_Microtrav ([Date_Create],[FIO_Lab]) " +
		...
		private void UpdateRow()
		{
			...
			using (SqlConnection connection = new SqlConnection(WC.ConnectionString))
			{
				...
				var Him_Podg_2_H2SO4 = dataGridView1.Rows[rowIndex].Cells["Him_Podg_2_H2SO4"].Value;
				var Him_Podg_2_H2O2 = dataGridView1.Rows[rowIndex].Cells["Him_Podg_2_H2O2"].Value;
				var Him_Podg_2_Cu2 = dataGridView1.Rows[rowIndex].Cells["Him_Podg_2_Cu2"].Value;
				...
				var Him_Podg_2_Correction_Mat = dataGridView1.Rows[rowIndex].Cells["Him_Podg_2_Correction_Mat"].Value;
				var Him_Podg_2_Correction_Score = dataGridView1.Rows[rowIndex].Cells["Him_Podg_2_Correction_Score"].Value;
				//var Comment = dataGridView1.Rows[e.RowIndex].Cells["Сomment"].Value;
				...
				string updateQuery = @"
            UPDATE Himich_Photorez_Microtrav
   SET [Him_Podg_2_H2SO4] = @Him_Podg_2_H2SO4
      ,[Him_Podg_2_H2O2] = @Him_Podg_2_H2O2
      ,[Him_Podg_2_Cu2] = @Him_Podg_2_Cu2,
[Him_Podg_2_podtrav] = @Him_Podg_2_podtrav,
[Him_Podg_2_pH] = @Him_Podg_2_pH,
