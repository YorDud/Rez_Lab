...
namespace Rez_Lab
{
	public partial class Start_Galvan_Line_MG_Cleaner_correcter : Form
	{
		...
		public Start_Galvan_Line_MG_Cleaner_correcter()
		{
			InitializeComponent();
			//TimerForButton();
			timer = new Timer();
		...
		private void Start_Galvan_Line_MG_Cleaner_correcter_Load(object sender, EventArgs e)
		{
...
//		}
		...
		{
			this.Hide();
			Galvan_Zat_Corrector tmc = new Galvan_Zat_Corrector();
			tmc.ShowDialog();
			this.Show();
		}
		...
		{
			this.Hide();
			Galvan_Line_MG_Corrector tmc = new Galvan_Line_MG_Corrector();
			tmc.ShowDialog();
			this.Show();
		}
		...
		{
			this.Hide();
			Galvan_Line_PAL_Corrector tmc = new Galvan_Line_PAL_Corrector();
			tmc.ShowDialog();
			this.Show();
		}
...
//		}
		...
		{
			this.Hide();
			Galvan_Line_MG_KisOch_Corrector tmc = new Galvan_Line_MG_KisOch_Corrector();
			tmc.ShowDialog();
			this.Show();
		}
		...
		{
			this.Hide();
			Galvan_Line_MG_Microtrav_Corrector tmc = new Galvan_Line_MG_Microtrav_Corrector();
			tmc.ShowDialog();
			this.Show();
		}
		...
		{
			this.Hide();
			Galvan_Line_MG_Cu_Corrector tmc = new Galvan_Line_MG_Cu_Corrector();
			tmc.ShowDialog();
			this.Show();
		}
		...
		{
			this.Hide();
			Galvan_Line_MG_Sn_Corrector tmc = new Galvan_Line_MG_Sn_Corrector();
			tmc.ShowDialog();
			this.Show();
		}
		...
		{
			this.Hide();
			Galvan_Line_MG_CuEl1819_Corrector tmc = new Galvan_Line_MG_CuEl1819_Corrector();
			tmc.ShowDialog();
			this.Show();
		}
		...
		{
			this.Hide();
			Galvan_Line_MG_CuEl2021_Corrector tmc = new Galvan_Line_MG_CuEl2021_Corrector();
			tmc.ShowDialog();
			this.Show();
		}
		...
		{
			this.Hide();
			Galvan_Line_MG_SnEl_Corrector tmc = new Galvan_Line_MG_SnEl_Corrector();
			tmc.ShowDialog();
			this.Show();
		}
		...
		private void Galvan_Line_MG_KisOch()
		{
			string query = @"
        SELECT COUNT(*) 
...
WHERE ([Сompleted] IS NULL OR [Сompleted] = '') 
AND (
    ([Gal_Line_1_Correction_Mat] IS NOT NULL AND [Gal_Line_1_Correction_Mat] <> '') OR ([Сomment] IS NOT NULL AND [Сomment] <> '') OR 
    ([Gal_Line_1_Correction_Score] IS NOT NULL AND [Gal_Line_1_Correction_Score] <> ''));";
		...
		private void Galvan_Line_MG_Microtrav()
		{
			string query = @"
        SELECT COUNT(*) 
...
WHERE ([Сompleted] IS NULL OR [Сompleted] = '') 
AND (
    ([Gal_Line_2_Correction_Mat] IS NOT NULL AND [Gal_Line_2_Correction_Mat] <> '') OR ([Сomment] IS NOT NULL AND [Сomment] <> '') OR 
    ([Gal_Line_2_Correction_Score] IS NOT NULL AND [Gal_Line_2_Correction_Score] <> ''));";
		...
		private void Galvan_Line_MG_Cu()
		{
			string query = @"
        SELECT COUNT(*) 
...
WHERE ([Сompleted] IS NULL OR [Сompleted] = '') 
AND (
    ([Gal_Line_3_Cu_Correction_Mat] IS NOT NULL AND [Gal_Line_3_Cu_Correction_Mat] <> '') OR ([Сomment] IS NOT NULL AND [Сomment] <> '') OR 
    ([Gal_Line_3_Cu_Correction_Score] IS NOT NULL AND [Gal_Line_3_Cu_Correction_Score] <> ''));";
		...
		private void Galvan_Line_MG_Sn()
		{
			string query = @"
        SELECT COUNT(*) 
...
WHERE ([Сompleted] IS NULL OR [Сompleted] = '') 
AND (
    ([Gal_Line_3_Sn_Correction_Mat] IS NOT NULL AND [Gal_Line_3_Sn_Correction_Mat] <> '') OR ([Сomment] IS NOT NULL AND [Сomment] <> '') OR 
    ([Gal_Line_3_Sn_Correction_Score] IS NOT NULL AND [Gal_Line_3_Sn_Correction_Score] <> ''));";
		...
		private void InitializeTimer3()
		{
			...
			// Настройка таймера
			blinkTimer3 = new Timer();
			blinkTimer3.Interval = 350; // 360 миллисекунд
			blinkTimer3.Tick += BlinkTimer_Tick3;
		}
		...
		private void Galvan_Line_MG_CuEl1819()
		{
			string query = @"
        SELECT COUNT(*) 
FROM Galvan_Line_MG_CuEl1819 
WHERE ([Сompleted] IS NULL OR [Сompleted] = '') and ([Gal_Line_4_Correction_Mat] <> 'NaCl' or [Gal_Line_4_Correction_Mat] is null)
AND (
    ([Gal_Line_4_Correction_Mat] IS NOT NULL AND [Gal_Line_4_Correction_Mat] <> '') OR ([Сomment] IS NOT NULL AND [Сomment] <> '') OR 
    ([Gal_Line_4_Correction_Score] IS NOT NULL AND [Gal_Line_4_Correction_Score] <> ''));";
		...
		private void InitializeTimer4()
		{
			...
			// Настройка таймера
			blinkTimer4 = new Timer();
			blinkTimer4.Interval = 350; // 470 миллисекунд
			blinkTimer4.Tick += BlinkTimer_Tick4;
		}
		...
		private void Galvan_Line_MG_CuEl2021()
		{
			string query = @"
        SELECT COUNT(*) 
FROM Galvan_Line_MG_CuEl2021 
WHERE ([Сompleted] IS NULL OR [Сompleted] = '') and ([Gal_Line_5_Correction_Mat] <> 'NaCl' or [Gal_Line_5_Correction_Mat] is null)
AND (
    ([Gal_Line_5_Correction_Mat] IS NOT NULL AND [Gal_Line_5_Correction_Mat] <> '') OR ([Сomment] IS NOT NULL AND [Сomment] <> '') OR 
    ([Gal_Line_5_Correction_Score] IS NOT NULL AND [Gal_Line_5_Correction_Score] <> ''));";
		...
		private void InitializeTimer5()
		{
			...
			// Настройка таймера
			blinkTimer5 = new Timer();
			blinkTimer5.Interval = 350; // 580 миллисекунд
			blinkTimer5.Tick += BlinkTimer_Tick5;
		}
		...
		private void Galvan_Line_MG_SnEl()
		{
			string query = @"
        SELECT COUNT(*) 
...
WHERE ([Сompleted] IS NULL OR [Сompleted] = '') 
AND (
    ([Gal_Line_6_Correction_Mat] IS NOT NULL AND [Gal_Line_6_Correction_Mat] <> '') OR ([Сomment] IS NOT NULL AND [Сomment] <> '') OR 
    ([Gal_Line_6_Correction_Score] IS NOT NULL AND [Gal_Line_6_Correction_Score] <> ''));";
		...
		private void InitializeTimer7()
		{
			...
			// Настройка таймера
			blinkTimer7 = new Timer();
			blinkTimer7.Interval = 350; // 790 миллисекунд
			blinkTimer7.Tick += BlinkTimer_Tick7;
		}
		...
		{
		"!!! Cleaner    Кислая очистка (ванна 9)!!!",
		"!!! Micro-Etch    Микротравление (ванна 12)!!! ",
		"!!! Pre-Dip Cu     Декапир Сu (ванна 17)!!!",
		"!!! Pre-Dip Sn     Декапир Sn (ванна 8)!!!",
		"!!! Plating Cu (ванна 18-19)    Cu электролит !!!",
		"!!! Plating Cu (ванна 20-21)    Cu электролит !!!",
		"!!! Plating Sn (ванна 7)    Sn электролит !!!",
		"!!! Rach-Strip    Травление подвесок (ванна 4)!!!"
		};
		...
		private DataSet GetAllDataFromDatabase()
		{
			...
			string query = @"
            SELECT 
                [Gal_Line_1_Correction_Mat],
                [Gal_Line_1_Correction_Score],
                [Сomment]
            FROM 
                Galvan_Line_MG_KisOch
            WHERE 
                ...
                AND (
                    ...
                    OR ([Сomment] IS NOT NULL AND [Сomment] <> '') OR ([Gal_Line_1_Correction_Score] IS NOT NULL AND [Gal_Line_1_Correction_Score] <> '')
                );

            SELECT 
                [Gal_Line_2_Correction_Mat],
                [Gal_Line_2_Correction_Score],
                [Сomment]
            FROM 
                Galvan_Line_MG_Microtrav
            WHERE 
                ...
                AND (
                    ...
                    OR ([Сomment] IS NOT NULL AND [Сomment] <> '') OR ([Gal_Line_2_Correction_Score] IS NOT NULL AND [Gal_Line_2_Correction_Score] <> '')
                );

            SELECT 
                [Gal_Line_3_Cu_Correction_Mat],
                [Gal_Line_3_Cu_Correction_Score],
                [Сomment]
            FROM 
                Galvan_Line_MG_Cu
            WHERE 
                ...
                AND (
                    ...
                    OR ([Сomment] IS NOT NULL AND [Сomment] <> '') OR ([Gal_Line_3_Cu_Correction_Score] IS NOT NULL AND [Gal_Line_3_Cu_Correction_Score] <> '')
                );

SELECT 
                [Gal_Line_3_Sn_Correction_Mat],
                [Gal_Line_3_Sn_Correction_Score],
                [Сomment]
            FROM 
                Galvan_Line_MG_Sn
            WHERE 
                ...
                AND (
                    ...
                    OR ([Сomment] IS NOT NULL AND [Сomment] <> '') OR ([Gal_Line_3_Sn_Correction_Score] IS NOT NULL AND [Gal_Line_3_Sn_Correction_Score] <> '')
                );

SELECT 
                [Gal_Line_4_Correction_Mat],
                [Gal_Line_4_Correction_Score],
                [Сomment]
            FROM 
                Galvan_Line_MG_CuEl1819
            WHERE 
...
SELECT 
                [Gal_Line_5_Correction_Mat],
                [Gal_Line_5_Correction_Score],
                [Сomment]
            FROM 
                Galvan_Line_MG_CuEl2021
            WHERE 
...
SELECT 
                [Gal_Line_6_Correction_Mat],
                [Gal_Line_6_Correction_Score],
                [Сomment]
            FROM 
                Galvan_Line_MG_SnEl
            WHERE 
...
SELECT 
                [Trav_Podv_Correction_Mat],
                [Trav_Podv_Correction_Score],
                [Сomment]
            FROM 
                Trav_Podv
            WHERE 
                ...
                AND (
                    ...
                    OR ([Сomment] IS NOT NULL AND [Сomment] <> '') OR ([Trav_Podv_Correction_Score] IS NOT NULL AND [Trav_Podv_Correction_Score] <> '')
                );
        ";
		...
		private void PrintDocument_PrintPage(object sender, PrintPageEventArgs e)
		{
			...
			string title = "Гальваническая линия 2-рой метализации - МГ";
		...
		{
			this.Hide();
			Trav_Podv_Corrector poa = new Trav_Podv_Corrector();
			poa.ShowDialog();
			this.Show();
		}
