...
{
	public partial class Zolot_HimNik_Technolog : Form
	{
		public Zolot_HimNik_Technolog()
		{
			InitializeComponent();
...
dataGridView1.Columns["Сomment"].HeaderText = "Комментарий";
		...
		private void Zolot_HimNik_Technolog_Load(object sender, EventArgs e)
		{
			LoadData();
...
[Zolot_Correction_Score] = @Zolot_Correction_Score
			...
			if (e.RowIndex >= 0 && e.ColumnIndex == 0)
			{
				...
				if (idValue != null)
				{
					if (int.TryParse(idValue.ToString(), out WC.id_click))
					{
						...
						string connectionString = WC.ConnectionString;
						string query = @"UPDATE Zolot_HimNik SET
                                 [Date_tech] = @Date_tech,
                                 [FIO_tech] = @FIO_tech
                                 WHERE ID = @Id";
		...
		private void HighlightEmptyCells(DataGridView dataGridView)
		{
			...
			int dateTechColumnIndex = 12; // Индекс столбца Date_tech (измените по необходимости)
