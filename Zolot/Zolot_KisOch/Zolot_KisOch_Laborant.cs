...
{
	public partial class Zolot_KisOch_Laborant : Form
	{
		public Zolot_KisOch_Laborant()
		{
			InitializeComponent();
...
dataGridView1.Columns["Сomment"].HeaderText = "Комментарий";
				...
				dataGridView1.Columns["Zolot_ZTW301"].DefaultCellStyle.BackColor = Color.LightBlue;
		...
		private void Zolot_KisOch_Laborant_Load(object sender, EventArgs e)
		{
			LoadData();
   ...
   SET [Zolot_ZTW301] = @Zolot_ZTW301,
	...
	WHERE ID = @ID";
		...
		private void NotClickOnTable()
		{
			...
			{
				if (column.Name != "Zolot_ZTW301" && column.Name != "Zolot_Cu2")
				{
		...
		private void dataGridView1_CellClick(object sender, DataGridViewCellEventArgs e)
		{
			if (e.RowIndex >= 0 && e.ColumnIndex == 3) // Проверяем, что ячейка выбрана
			{
				...
				using (Form inputForm = new Form())
				{
					...
					inputForm.Text = "ZTW-301";


					Label labelValue1 = new Label()
					{
						Text = "C(ZTW-310) = a × 7,94 × T ",
						Top = 10,
						Left = 30,
						Width = 330
					};
					...
					calculateButton.Click += (s, args) =>
					{
						...
						{
							double result = Math.Round(v1 * 7.94 * v2, 2);
			...
			else if (e.RowIndex >= 0 && e.ColumnIndex == 4) // Проверяем, что ячейка выбрана
			{
				...
				using (Form inputForm = new Form())
				{
					...
					calculateButton.Click += (s, args) =>
					{
						...
						{
							double result = Math.Round(v1 * 0.159, 2);
