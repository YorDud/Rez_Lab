...
{
	public partial class Zolot_KisOch_Technolog : Form
	{
		public Zolot_KisOch_Technolog()
		{
			InitializeComponent();
			////label11.Text = "Na2S2O8 - 80 г/л\r\nОБЪЕМ ВАННЫ: 460 л";
		...
		private void LoadData()
		{
			...
			{
				connection.Open();
				dataAdapter = new SqlDataAdapter("SELECT * FROM Zolot_KisOch ORDER BY Date_Create DESC", connection);
				...
				//
				dataGridView1.Columns["Zolot_ZTW301"].HeaderText = "ZTW-301";
...
dataGridView1.Columns["Сomment"].HeaderText = "Комментарий";
				...
				dataGridView1.Columns["Zolot_Correction_Mat"].DefaultCellStyle.BackColor = Color.PeachPuff;
				dataGridView1.Columns["Zolot_Correction_Score"].DefaultCellStyle.BackColor = Color.PeachPuff;
		...
		private void Zolot_KisOch_Technolog_Load(object sender, EventArgs e)
		{
			LoadData();
		...
		private void NotClickOnTable()
		{
			...
			{
				if (column.Name != "Zolot_Correction_Mat" && column.Name != "Zolot_Correction_Score" && column.Name != "Сomment") // предполагается, что "ID" - это столбец, который можно редактировать
				{
		...
		private void dataGridView1_CellEndEdit(object sender, DataGridViewCellEventArgs e)
		{
			...
			using (SqlConnection connection = new SqlConnection(WC.ConnectionString))
			{
				...
				var Zolot_ZTW301 = dataGridView1.Rows[e.RowIndex].Cells["Zolot_ZTW301"].Value;
				...
				string updateQuery = @"
        UPDATE Zolot_KisOch SET
            [Date_tech] = @Date_tech,
            [Zolot_ZTW301] = @Zolot_ZTW301,
      [Zolot_Cu2] = @Zolot_Cu2,
...
[Zolot_Correction_Score] = @Zolot_Correction_Score
        ...
        WHERE ID = @Id";
				...
				using (SqlCommand command = new SqlCommand(updateQuery, connection))
				{
					...
					command.Parameters.AddWithValue("@Zolot_ZTW301", Zolot_ZTW301);
		...
		private void LoadDataDateTimePicker(DateTime date)
		{
			using (SqlConnection connection = new SqlConnection(WC.ConnectionString))
			{
				...
				string query = "SELECT * FROM Zolot_KisOch WHERE CAST(Date_Create AS DATE) = @SelectedDate ORDER BY Date_Create DESC";
				using (SqlCommand command = new SqlCommand(query, connection))
				{
		...
		private void LoadDataByDateRange(DateTime startDate, DateTime endDate)
		{
			...
			{
				connection.Open();
				string query = "SELECT * FROM Zolot_KisOch WHERE CAST(Date_Create AS DATE) BETWEEN @StartDate AND @EndDate ORDER BY Date_Create DESC";
				using (SqlCommand command = new SqlCommand(query, connection))
				{
			...
			if (e.RowIndex >= 0 && e.ColumnIndex == 0)
			{
				...
				if (idValue != null)
				{
					if (int.TryParse(idValue.ToString(), out WC.id_click))
					{
						...
						string connectionString = WC.ConnectionString;
						string query = @"UPDATE Zolot_KisOch SET
                                 [Date_tech] = @Date_tech,
                                 [FIO_tech] = @FIO_tech
                                 WHERE ID = @Id";
		...
		private void HighlightEmptyCells(DataGridView dataGridView)
		{
			...
			int dateTechColumnIndex = 8; // Индекс столбца Date_tech (измените по необходимости)
		...
		private void Add_Row()
		{
			using (SqlConnection connection = new SqlConnection(WC.ConnectionString))
			{
				...
				string sqlInsert = "INSERT INTO Zolot_KisOch ([Date_Create],[FIO_Lab]) " +
		...
		private void UpdateRow()
		{
			...
			using (SqlConnection connection = new SqlConnection(WC.ConnectionString))
			{
				...
				var Zolot_ZTW301 = dataGridView1.Rows[rowIndex].Cells["Zolot_ZTW301"].Value;
				var Zolot_Cu2 = dataGridView1.Rows[rowIndex].Cells["Zolot_Cu2"].Value;

				var Zolot_Correction_Mat = dataGridView1.Rows[rowIndex].Cells["Zolot_Correction_Mat"].Value;
				var Zolot_Correction_Score = dataGridView1.Rows[rowIndex].Cells["Zolot_Correction_Score"].Value;
