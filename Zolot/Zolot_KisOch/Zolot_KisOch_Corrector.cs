...
{
	public partial class Zolot_KisOch_Corrector : Form
	{
		public Zolot_KisOch_Corrector()
		{
			InitializeComponent();
		...
		private void LoadData()
		{
			...
			{
				connection.Open();
				dataAdapter = new SqlDataAdapter("SELECT ID, Zolot_Correction_Mat, Zolot_Correction_Score, FIO_tech, Date_tech, FIO_Corr, Date_Corr, Сompleted,[Start_corr],[Date_start_corr],[FIO_start_corr],[Сomment] FROM Zolot_KisOch ORDER BY Date_Create DESC", connection);
				...
				////
				//dataGridView1.Columns["Black_Hole_7_Na2S2O3"].HeaderText = "Na2S2O8";
...
HighlightEmptyCells(dataGridView1, columnsToCheck);
		...
		private void Zolot_KisOch_Corrector_Load(object sender, EventArgs e)
		{
			LoadData();
		...
		private void LoadDataDateTimePicker(DateTime date)
		{
			using (SqlConnection connection = new SqlConnection(WC.ConnectionString))
			{
				...
				string query = "SELECT ID, Zolot_Correction_Mat, Zolot_Correction_Score, FIO_tech, Date_tech, FIO_Corr, Date_Corr, Сompleted,[Start_corr],[Date_start_corr],[FIO_start_corr],[Сomment] FROM Zolot_KisOch WHERE CAST(Date_Create AS DATE) = @SelectedDate ORDER BY Date_Create DESC";
		...
		private void HighlightEmptyCells(DataGridView dataGridView, int[] columns)
{
	foreach (DataGridViewRow row in dataGridView.Rows)
	{
		...
		private void LoadDataByDateRange(DateTime startDate, DateTime endDate)
		{
			...
			{
				connection.Open();
				string query = "SELECT ID, Zolot_Correction_Mat, Zolot_Correction_Score, FIO_tech, Date_tech, FIO_Corr, Date_Corr, Сompleted,[Start_corr],[Date_start_corr],[FIO_start_corr],[Сomment] FROM Zolot_KisOch WHERE CAST(Date_Create AS DATE) BETWEEN @StartDate AND @EndDate ORDER BY Date_Create DESC";
				using (SqlCommand command = new SqlCommand(query, connection))
				{
		...
		private void dataGridView1_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
		{
			...
			if (e.RowIndex >= 0 && e.ColumnIndex == 7)
			{
				...
				var Correction_Mat_Value = selectedRow.Cells["Zolot_Correction_Mat"].Value;
				var Correction_Score_Value = selectedRow.Cells["Zolot_Correction_Score"].Value;
