...
{
	public partial class Zolot_PredActiv_Laborant : Form
	{
		public Zolot_PredActiv_Laborant()
		{
			InitializeComponent();
...
dataGridView1.Columns["Сomment"].HeaderText = "Комментарий";
		...
		private void Zolot_PredActiv_Laborant_Load(object sender, EventArgs e)
		{
			LoadData();
			...
			if (e.RowIndex >= 0 && e.ColumnIndex == 3) // Проверяем, что ячейка выбрана
			{
				...
				using (Form inputForm = new Form())
				{
					...
					calculateButton.Click += (s, args) =>
					{
						...
						{
							double result = Math.Round(v1 * 5.4 * v2, 2); // Пример формулы (сложение)
