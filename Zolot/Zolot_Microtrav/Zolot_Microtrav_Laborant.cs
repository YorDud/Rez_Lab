...
{
	public partial class Zolot_Microtrav_Laborant : Form
	{
		public Zolot_Microtrav_Laborant()
		{
			InitializeComponent();
		...
		private void LoadData()
		{
			...
			{
				connection.Open();
				dataAdapter = new SqlDataAdapter("SELECT * FROM Zolot_Microtrav ORDER BY Date_Create DESC", connection);
				...
				dataGridView1.Columns["Zolot_ZTW806"].HeaderText = "ZTW-806";
				dataGridView1.Columns["Zolot_Cu2"].HeaderText = "Cu2+";
				dataGridView1.Columns["Zolot_StepTrav"].HeaderText = "степень травления";
...
dataGridView1.Columns["Сomment"].HeaderText = "Комментарий";
				...
				dataGridView1.Columns["Zolot_ZTW806"].DefaultCellStyle.BackColor = Color.LightBlue;
				dataGridView1.Columns["Zolot_Cu2"].DefaultCellStyle.BackColor = Color.LightBlue;
				dataGridView1.Columns["Zolot_StepTrav"].DefaultCellStyle.BackColor = Color.LightBlue;
		...
		private void Zolot_Microtrav_Laborant_Load(object sender, EventArgs e)
		{
			LoadData();
		...
		private void LoadDataDateTimePicker(DateTime date)
		{
			using (SqlConnection connection = new SqlConnection(WC.ConnectionString))
			{
				...
				string query = "SELECT * FROM Zolot_Microtrav WHERE CAST(Date_Create AS DATE) = @SelectedDate ORDER BY Date_Create DESC";
		...
		private void LoadDataByDateRange(DateTime startDate, DateTime endDate)
		{
			...
			{
				connection.Open();
				string query = "SELECT * FROM Zolot_Microtrav WHERE CAST(Date_Create AS DATE) BETWEEN @StartDate AND @EndDate ORDER BY Date_Create DESC";
				using (SqlCommand command = new SqlCommand(query, connection))
				{
		...
		private void Add_Row()
		{
			using (SqlConnection connection = new SqlConnection(WC.ConnectionString))
			{
				...
				string sqlInsert = "INSERT INTO Zolot_Microtrav ([Date_Create],[FIO_Lab]) " +
		...
		private void UpdateRow()
		{
			...
			using (SqlConnection connection = new SqlConnection(WC.ConnectionString))
			{
				...
				var Zolot_ZTW806 = dataGridView1.Rows[rowIndex].Cells["Zolot_ZTW806"].Value;
				var Zolot_Cu2 = dataGridView1.Rows[rowIndex].Cells["Zolot_Cu2"].Value;
				var Zolot_StepTrav = dataGridView1.Rows[rowIndex].Cells["Zolot_StepTrav"].Value;



				// Создаем команду обновления с несколькими столбцами
				string updateQuery = @"
            UPDATE Zolot_Microtrav
   SET [Zolot_H2SO4] = @Zolot_H2SO4
      ,[Zolot_ZTW806] = @Zolot_ZTW806
      ,[Zolot_Cu2] = @Zolot_Cu2,
[Zolot_StepTrav] = @Zolot_StepTrav
	...
	WHERE ID = @ID";
				...
				using (SqlCommand command = new SqlCommand(updateQuery, connection))
				{
					...
					command.Parameters.AddWithValue("@Zolot_ZTW806", Zolot_ZTW806);
					command.Parameters.AddWithValue("@Zolot_Cu2", Zolot_Cu2);
					command.Parameters.AddWithValue("@Zolot_StepTrav", Zolot_StepTrav);
					//
		...
		private void NotClickOnTable()
		{
			...
			{
				if (column.Name != "Zolot_H2SO4" && column.Name != "Zolot_ZTW806" && column.Name != "Zolot_Cu2" && column.Name != "Zolot_StepTrav")
				{
		...
		private void dataGridView1_CellEndEdit(object sender, DataGridViewCellEventArgs e)
		{
			...
			using (SqlConnection connection = new SqlConnection(WC.ConnectionString))
			{
				...
				var Zolot_ZTW806 = dataGridView1.Rows[e.RowIndex].Cells["Zolot_ZTW806"].Value;
				var Zolot_Cu2 = dataGridView1.Rows[e.RowIndex].Cells["Zolot_Cu2"].Value;
				var Zolot_StepTrav = dataGridView1.Rows[e.RowIndex].Cells["Zolot_StepTrav"].Value;
...
[Zolot_StepTrav] = @Zolot_StepTrav
	...
	WHERE ID = @ID";
		...
		{
			if (e.RowIndex >= 0 && e.ColumnIndex == 6) // Проверяем, что ячейка выбрана
			{
			...
			else if (e.RowIndex >= 0 && e.ColumnIndex == 3) // Проверяем, что ячейка выбрана
			{
				...
				using (Form inputForm = new Form())
				{
					...
					calculateButton.Click += (s, args) =>
					{
						...
						{
							double result = Math.Round(v1 * 1.34, 2); // Пример формулы (сложение)
			...
			else if (e.RowIndex >= 0 && e.ColumnIndex == 5) // Проверяем, что ячейка выбрана
			{
				...
				using (Form inputForm = new Form())
				{
					...
					calculateButton.Click += (s, args) =>
					{
						...
						if (double.TryParse(value1.Text, out double v1))
						{
							double result = Math.Round(v1 * 1.59, 2); // Пример формулы (сложение)
			...
			else if (e.RowIndex >= 0 && e.ColumnIndex == 4) // Проверяем, что ячейка выбрана
			{
				...
				using (Form inputForm = new Form())
				{
					...
					inputForm.MinimizeBox = false;
					inputForm.StartPosition = FormStartPosition.CenterParent;
					inputForm.Text = "ZTW-806";

					Label labelValue1 = new Label()
					{
						Text = "C(ZTW-310) = (a × 8,875 – 2,79 × С(Сu2+)) × 5",
						Top = 10,
						Left = 30,
						Width = 330
					};
					...
					Label labelValueA = new Label()
					{
						Text = "a – количество 0,1Н раствора Na2S2O3, израсходованного на титрование, мл.\r\nС(Сu2+) – концентрация меди в растворе по анализу",
						Top = 40,
						Left = 10,
						Width = 330,
						Height = 100
					...
					calculateButton.Click += (s, args) =>
					{
						...
						if (double.TryParse(value1.Text, out double v1) && double.TryParse(value2.Text, out double v2))
						{
							double result = Math.Round((v1 * 8.875 - 2.79 * v2) * 5, 2); // Пример формулы (сложение)
