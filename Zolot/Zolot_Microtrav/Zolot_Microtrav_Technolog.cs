...
{
	public partial class Zolot_Microtrav_Technolog : Form
	{
		public Zolot_Microtrav_Technolog()
		{
			InitializeComponent();
...
dataGridView1.Columns["Сomment"].HeaderText = "Комментарий";
		...
		private void Zolot_Microtrav_Technolog_Load(object sender, EventArgs e)
		{
			LoadData();
			...
			if (e.RowIndex >= 0 && e.ColumnIndex == 0)
			{
				...
				if (idValue != null)
				{
					if (int.TryParse(idValue.ToString(), out WC.id_click))
					{
						...
						string connectionString = WC.ConnectionString;
						string query = @"UPDATE Zolot_Microtrav SET
                                 [Date_tech] = @Date_tech,
                                 [FIO_tech] = @FIO_tech
                                 WHERE ID = @Id";
