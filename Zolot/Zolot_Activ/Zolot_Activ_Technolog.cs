...
{
	public partial class Zolot_Activ_Technolog : Form
	{
		public Zolot_Activ_Technolog()
		{
			InitializeComponent();
...
dataGridView1.Columns["Сomment"].HeaderText = "Комментарий";
		...
		private void Zolot_Activ_Technolog_Load(object sender, EventArgs e)
		{
			LoadData();
		...
		private void NotClickOnTable()
		{
			...
			{
				if (column.Name != "Zolot_Correction_Score" && column.Name != "Zolot_Correction_Mat" && column.Name != "Сomment") // предполагается, что "ID" - это столбец, который можно редактировать
				{
					column.ReadOnly = true;
				}
			}
		}
		//int[] columnsToCheck = { 5, 8, 12, 16, 20, 23, 25, 27 };
...
[Zolot_Correction_Score] = @Zolot_Correction_Score
			...
			if (e.RowIndex >= 0 && e.ColumnIndex == 0)
			{
				...
				if (idValue != null)
				{
					if (int.TryParse(idValue.ToString(), out WC.id_click))
					{
						...
						string connectionString = WC.ConnectionString;
						string query = @"UPDATE Zolot_Activ SET
                                 [Date_tech] = @Date_tech,
                                 [FIO_tech] = @FIO_tech
                                 WHERE ID = @Id";
		...
		private void UpdateRow()
		{
			...
			using (SqlConnection connection = new SqlConnection(WC.ConnectionString))
			{
				...
				string updateQuery = @"
            UPDATE Zolot_Activ
   SET [Zolot_Correction_Mat] = @Zolot_Correction_Mat,
[Zolot_Correction_Score] = @Zolot_Correction_Score
