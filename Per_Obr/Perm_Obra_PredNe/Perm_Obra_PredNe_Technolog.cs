...
{
	public partial class Perm_Obra_PredNe_Technolog : Form
	{
		public Perm_Obra_PredNe_Technolog()
		{
			InitializeComponent();
...
dataGridView1.Columns["Сomment"].HeaderText = "Комментарий";
				...
				dataGridView1.Columns["Per_Obr_3_Correction_Mat"].DefaultCellStyle.BackColor = Color.PeachPuff;
				dataGridView1.Columns["Per_Obr_3_Correction_Score"].DefaultCellStyle.BackColor = Color.PeachPuff;
		...
		private void Perm_Obra_PredNe_Technolog_Load(object sender, EventArgs e)
		{
			LoadData();
		...
		private void NotClickOnTable()
		{
			...
			{
				if (column.Name != "Per_Obr_3_Correction_Score" && column.Name != "Per_Obr_3_Correction_Mat" && column.Name != "Сomment") // предполагается, что "ID" - это столбец, который можно редактировать
				{
		...
		private void dataGridView1_CellEndEdit(object sender, DataGridViewCellEventArgs e)
		{
			...
			using (SqlConnection connection = new SqlConnection(WC.ConnectionString))
			{
				...
				var Per_Obr_3_Correction_Mat = dataGridView1.Rows[e.RowIndex].Cells["Per_Obr_3_Correction_Mat"].Value;
				var Per_Obr_3_Correction_Score = dataGridView1.Rows[e.RowIndex].Cells["Per_Obr_3_Correction_Score"].Value;
				...
				string updateQuery = @"
        UPDATE Perm_Obra_PredNe SET
            [Date_tech] = @Date_tech,
            [Per_Obr_3_Correction_Mat] = @Per_Obr_3_Correction_Mat
			,[Per_Obr_3_Correction_Score] = @Per_Obr_3_Correction_Score
			,[Сomment] = @Comment
            ,FIO_tech = @FIO_tech
        WHERE ID = @Id";
				...
				using (SqlCommand command = new SqlCommand(updateQuery, connection))
				{
					...
					command.Parameters.AddWithValue("@Per_Obr_3_Correction_Mat", Per_Obr_3_Correction_Mat);
					command.Parameters.AddWithValue("@Per_Obr_3_Correction_Score", Per_Obr_3_Correction_Score);
			...
			if (e.RowIndex >= 0 && e.ColumnIndex == 0)
			{
				...
				if (idValue != null)
				{
					if (int.TryParse(idValue.ToString(), out WC.id_click))
					{
						...
						string connectionString = WC.ConnectionString;
						string query = @"UPDATE Perm_Obra_PredNe SET
                                 [Date_tech] = @Date_tech,
                                 [FIO_tech] = @FIO_tech
                                 WHERE ID = @Id";
		...
		private void UpdateRow()
		{
			...
			using (SqlConnection connection = new SqlConnection(WC.ConnectionString))
			{
				...
				var Per_Obr_3_Correction_Mat = dataGridView1.Rows[rowIndex].Cells["Per_Obr_3_Correction_Mat"].Value;
				var Per_Obr_3_Correction_Score = dataGridView1.Rows[rowIndex].Cells["Per_Obr_3_Correction_Score"].Value;
				//var Comment = dataGridView1.Rows[e.RowIndex].Cells["Сomment"].Value;
   ...
   SET [Per_Obr_3_Correction_Mat] = @Per_Obr_3_Correction_Mat
			,[Per_Obr_3_Correction_Score] = @Per_Obr_3_Correction_Score
