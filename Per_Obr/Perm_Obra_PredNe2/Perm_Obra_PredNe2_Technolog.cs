...
{
	public partial class Perm_Obra_PredNe2_Technolog : Form
	{
		public Perm_Obra_PredNe2_Technolog()
		{
			InitializeComponent();
			//label11.Text = "Н2SO4 - 7 - 11 %\r\nDES 430  - 8 - 12 %\r\nОБЪЕМ ВАННЫ: 180 л";
		...
		private void LoadData()
		{
			...
			{
				connection.Open();
				dataAdapter = new SqlDataAdapter("SELECT * FROM Perm_Obra_PredNe2 ORDER BY Date_Create DESC", connection);
				...
				//
				dataGridView1.Columns["Per_Obr_4_Н2SO4"].HeaderText = "Н2SO4";
				//dataGridView1.Columns["Per_Obr_4_DES430"].HeaderText = "DES 430";
				dataGridView1.Columns["Per_Obr_4_DES430"].HeaderText = "413Б";
				dataGridView1.Columns["Per_Obr_4_Correction_Mat"].HeaderText = "Корректировка Материал";
				dataGridView1.Columns["Per_Obr_4_Correction_Score"].HeaderText = "Корректировка Количество";
				//
...
dataGridView1.Columns["Сomment"].HeaderText = "Комментарий";
				...
				dataGridView1.Columns["Per_Obr_4_Correction_Mat"].DefaultCellStyle.BackColor = Color.PeachPuff;
				dataGridView1.Columns["Per_Obr_4_Correction_Score"].DefaultCellStyle.BackColor = Color.PeachPuff;
		...
		private void Perm_Obra_PredNe2_Technolog_Load(object sender, EventArgs e)
		{
			LoadData();
		...
		private void NotClickOnTable()
		{
			...
			{
				if (column.Name != "Per_Obr_4_Correction_Score" && column.Name != "Per_Obr_4_Correction_Mat" && column.Name != "Сomment") // предполагается, что "ID" - это столбец, который можно редактировать
				{
		...
		private void dataGridView1_CellEndEdit(object sender, DataGridViewCellEventArgs e)
		{
			...
			using (SqlConnection connection = new SqlConnection(WC.ConnectionString))
			{
				...
				var Per_Obr_4_Correction_Mat = dataGridView1.Rows[e.RowIndex].Cells["Per_Obr_4_Correction_Mat"].Value;
				var Per_Obr_4_Correction_Score = dataGridView1.Rows[e.RowIndex].Cells["Per_Obr_4_Correction_Score"].Value;
				...
				string updateQuery = @"
        UPDATE Perm_Obra_PredNe2 SET
            [Date_tech] = @Date_tech,
            [Per_Obr_4_Correction_Mat] = @Per_Obr_4_Correction_Mat
			,[Per_Obr_4_Correction_Score] = @Per_Obr_4_Correction_Score
			,[Сomment] = @Comment
            ,FIO_tech = @FIO_tech
        WHERE ID = @Id";
				...
				using (SqlCommand command = new SqlCommand(updateQuery, connection))
				{
					...
					command.Parameters.AddWithValue("@Per_Obr_4_Correction_Mat", Per_Obr_4_Correction_Mat);
					command.Parameters.AddWithValue("@Per_Obr_4_Correction_Score", Per_Obr_4_Correction_Score);
		...
		private void LoadDataDateTimePicker(DateTime date)
		{
			using (SqlConnection connection = new SqlConnection(WC.ConnectionString))
			{
				...
				string query = "SELECT * FROM Perm_Obra_PredNe2 WHERE CAST(Date_Create AS DATE) = @SelectedDate ORDER BY Date_Create DESC";
				using (SqlCommand command = new SqlCommand(query, connection))
				{
		...
		private void LoadDataByDateRange(DateTime startDate, DateTime endDate)
		{
			...
			{
				connection.Open();
				string query = "SELECT * FROM Perm_Obra_PredNe2 WHERE CAST(Date_Create AS DATE) BETWEEN @StartDate AND @EndDate ORDER BY Date_Create DESC";
				using (SqlCommand command = new SqlCommand(query, connection))
				{
			...
			if (e.RowIndex >= 0 && e.ColumnIndex == 0)
			{
				...
				if (idValue != null)
				{
					if (int.TryParse(idValue.ToString(), out WC.id_click))
					{
						...
						string connectionString = WC.ConnectionString;
						string query = @"UPDATE Perm_Obra_PredNe2 SET
                                 [Date_tech] = @Date_tech,
                                 [FIO_tech] = @FIO_tech
                                 WHERE ID = @Id";
		...
		private void Add_Row()
		{
			using (SqlConnection connection = new SqlConnection(WC.ConnectionString))
			{
				...
				string sqlInsert = "INSERT INTO Perm_Obra_PredNe2 ([Date_Create],[FIO_Lab]) " +
		...
		private void UpdateRow()
		{
			...
			using (SqlConnection connection = new SqlConnection(WC.ConnectionString))
			{
				...
				var Per_Obr_4_Correction_Mat = dataGridView1.Rows[rowIndex].Cells["Per_Obr_4_Correction_Mat"].Value;
				var Per_Obr_4_Correction_Score = dataGridView1.Rows[rowIndex].Cells["Per_Obr_4_Correction_Score"].Value;
				//var Comment = dataGridView1.Rows[e.RowIndex].Cells["Сomment"].Value;
				...
				string updateQuery = @"
            UPDATE Perm_Obra_PredNe2
   SET [Per_Obr_4_Correction_Mat] = @Per_Obr_4_Correction_Mat
			,[Per_Obr_4_Correction_Score] = @Per_Obr_4_Correction_Score
