...
{
	public partial class Himich_Podgotov_Dek_Laborant : Form
	{
		public Himich_Podgotov_Dek_Laborant()
		{
			InitializeComponent();
		...
		private void LoadData()
		{
			...
			{
				connection.Open();
				dataAdapter = new SqlDataAdapter("SELECT * FROM Himich_Podgotov_Dek ORDER BY Date_Create DESC", connection);
				...
				//
				dataGridView1.Columns["Him_Podg_3_plot"].HeaderText = "HCl";
...
dataGridView1.Columns["Сomment"].HeaderText = "Комментарий";
				...
				dataGridView1.Columns["Him_Podg_3_plot"].DefaultCellStyle.BackColor = Color.LightBlue;
				dataGridView1.Columns["Him_Podg_3_Cu2"].DefaultCellStyle.BackColor = Color.LightBlue;
		...
		private void Himich_Podgotov_Dek_Laborant_Load(object sender, EventArgs e)
		{
			LoadData();
		...
		private void LoadDataDateTimePicker(DateTime date)
		{
			using (SqlConnection connection = new SqlConnection(WC.ConnectionString))
			{
				...
				string query = "SELECT * FROM Himich_Podgotov_Dek WHERE CAST(Date_Create AS DATE) = @SelectedDate ORDER BY Date_Create DESC";
		...
		private void LoadDataByDateRange(DateTime startDate, DateTime endDate)
		{
			...
			{
				connection.Open();
				string query = "SELECT * FROM Himich_Podgotov_Dek WHERE CAST(Date_Create AS DATE) BETWEEN @StartDate AND @EndDate ORDER BY Date_Create DESC";
				using (SqlCommand command = new SqlCommand(query, connection))
				{
		...
		private void Add_Row()
		{
			using (SqlConnection connection = new SqlConnection(WC.ConnectionString))
			{
				...
				string sqlInsert = "INSERT INTO Himich_Podgotov_Dek ([Date_Create],[FIO_Lab]) " +
		...
		private void UpdateRow()
		{
			...
			using (SqlConnection connection = new SqlConnection(WC.ConnectionString))
			{
				...
				var Him_Podg_3_plot = dataGridView1.Rows[rowIndex].Cells["Him_Podg_3_plot"].Value;
				var Him_Podg_3_Cu2 = dataGridView1.Rows[rowIndex].Cells["Him_Podg_3_Cu2"].Value;



				// Создаем команду обновления с несколькими столбцами
				string updateQuery = @"
            UPDATE Himich_Podgotov_Dek
   SET [Him_Podg_3_plot] = @Him_Podg_3_plot, 
[Him_Podg_3_Cu2] = @Him_Podg_3_Cu2
	...
	WHERE ID = @ID";
				...
				using (SqlCommand command = new SqlCommand(updateQuery, connection))
				{
					...
					//
					command.Parameters.AddWithValue("@Him_Podg_3_plot", Him_Podg_3_plot);
					command.Parameters.AddWithValue("@Him_Podg_3_Cu2", Him_Podg_3_Cu2);
		...
		private void NotClickOnTable()
		{
			...
			{
				if (column.Name != "Him_Podg_3_plot" && column.Name != "Him_Podg_3_Cu2")
				{
		...
		private void dataGridView1_CellEndEdit(object sender, DataGridViewCellEventArgs e)
		{
			...
			using (SqlConnection connection = new SqlConnection(WC.ConnectionString))
			{
				...
				var Him_Podg_3_plot = dataGridView1.Rows[e.RowIndex].Cells["Him_Podg_3_plot"].Value;
				var Him_Podg_3_Cu2 = dataGridView1.Rows[e.RowIndex].Cells["Him_Podg_3_Cu2"].Value;
		...
		private void dataGridView1_CellClick(object sender, DataGridViewCellEventArgs e)
		{
			if (e.RowIndex >= 0 && e.ColumnIndex == 3) // Проверяем, что ячейка выбрана
			{
				...
				using (Form inputForm = new Form())
				{
					...
					{
						Text = "a – количество 1Н раствора NaOH, израсходованного на титрование, мл;\r\nT – титр 1Н раствора NaOH по янтарной кислоте, г/мл",
						Top = 40,
						Left = 10,
						Width = 330,
						Height = 100
					...
					calculateButton.Click += (s, args) =>
					{
						...
						{
							double result = Math.Round(v1 * 0.1 * v2, 2); // Пример формулы (сложение)
			...
			else if (e.RowIndex >= 0 && e.ColumnIndex == 4) // Проверяем, что ячейка выбрана
			{
				...
				using (Form inputForm = new Form())
				{
					...
					Label labelValueA = new Label()
					{
						Text = "а - количество 0,1Н раствора Трилона Б, израсходованного на титрование, мл",
						Top = 40,
						Left = 10,
						Width = 330,
						Height = 100
