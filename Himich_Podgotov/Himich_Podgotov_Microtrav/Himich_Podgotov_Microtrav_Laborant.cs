...
{
	public partial class Himich_Podgotov_Microtrav_Laborant : Form
	{
		public Himich_Podgotov_Microtrav_Laborant()
		{
			InitializeComponent();
		...
		private void LoadData()
		{
			...
			{
				connection.Open();
				dataAdapter = new SqlDataAdapter("SELECT * FROM Himich_Podgotov_Microtrav ORDER BY Date_Create DESC", connection);
				...
				//
				dataGridView1.Columns["Him_Podg_2_Ci2"].HeaderText = "Cu 2+";
...
dataGridView1.Columns["Сomment"].HeaderText = "Комментарий";
		...
		private void Himich_Podgotov_Microtrav_Laborant_Load(object sender, EventArgs e)
		{
			LoadData();
		...
		private void LoadDataDateTimePicker(DateTime date)
		{
			using (SqlConnection connection = new SqlConnection(WC.ConnectionString))
			{
				...
				string query = "SELECT * FROM Himich_Podgotov_Microtrav WHERE CAST(Date_Create AS DATE) = @SelectedDate ORDER BY Date_Create DESC";
		...
		private void LoadDataByDateRange(DateTime startDate, DateTime endDate)
		{
			...
			{
				connection.Open();
				string query = "SELECT * FROM Himich_Podgotov_Microtrav WHERE CAST(Date_Create AS DATE) BETWEEN @StartDate AND @EndDate ORDER BY Date_Create DESC";
				using (SqlCommand command = new SqlCommand(query, connection))
				{
		...
		private void Add_Row()
		{
			using (SqlConnection connection = new SqlConnection(WC.ConnectionString))
			{
				...
				string sqlInsert = "INSERT INTO Himich_Podgotov_Microtrav ([Date_Create],[FIO_Lab]) " +
		...
		private void UpdateRow()
		{
			...
			using (SqlConnection connection = new SqlConnection(WC.ConnectionString))
			{
				...
				string updateQuery = @"
            UPDATE Himich_Podgotov_Microtrav
   SET [Him_Podg_2_Ci2] = @Him_Podg_2_Ci2,
...
[Him_Podg_2_pH] = @Him_Podg_2_pH
	...
	WHERE ID = @ID";
		...
		private void NotClickOnTable()
		{
			...
			{
				if (column.Name != "Him_Podg_2_Ci2" && column.Name != "Him_Podg_2_podtrav" && column.Name != "Him_Podg_2_kislot" && column.Name != "Him_Podg_2_pH")
				{
		...
		private void dataGridView1_CellClick(object sender, DataGridViewCellEventArgs e)
		{
			if (e.RowIndex >= 0 && e.ColumnIndex == 5) // Проверяем, что ячейка выбрана
			{
				...
				using (Form inputForm = new Form())
				{
					...
					calculateButton.Click += (s, args) =>
					{
						...
						{
							double result = Math.Round(v1 * 0.1, 2); // Пример формулы (сложение)
			...
			else if (e.RowIndex >= 0 && e.ColumnIndex == 4) // Проверяем, что ячейка выбрана
			{
				...
				using (Form inputForm = new Form())
				{
					...
					Label labelValueA = new Label()
					{
						Text = "w1 - вес образца фольгир-го диэлектрика размером 10x10 см до обработки\r\nw2 - вес образца фольгир-го диэлектрика размером 10x10 см после обработки",
						Top = 40,
						Left = 10,
						Width = 330,
						Height = 100
