...
{
	public partial class Oxide_Oxidir_Technolog : Form
	{
		public Oxide_Oxidir_Technolog()
		{
			InitializeComponent();
			//label11.Text = "Сu2+ - 15-60 г/л\r\nподтрав - 07-1,6 мкм\r\nкислотность > 1,3 моль/л\r\nBROX1100 - 2-5\r\nОБЪЕМ ВАННЫ: 290 - 385 л";
		...
		private void LoadData()
		{
			...
			{
				connection.Open();
				dataAdapter = new SqlDataAdapter("SELECT * FROM Oxide_Oxidir ORDER BY Date_Create DESC", connection);
				...
				dataGridView1.Columns["Oxide_BROX1100"].HeaderText = "BROX-1100";
				dataGridView1.Columns["Oxide_H2SO4"].HeaderText = "H2SO4";
				dataGridView1.Columns["Oxide_H2O2"].HeaderText = "H2O2";
				dataGridView1.Columns["Oxide_Cu2"].HeaderText = "Cu 2+";
				dataGridView1.Columns["Oxide_podtrav"].HeaderText = "Подтрав";
				dataGridView1.Columns["Oxide_Correction_Mat"].HeaderText = "Корректировка материал";
				dataGridView1.Columns["Oxide_Correction_Score"].HeaderText = "Корректировка количество";
				//
...
dataGridView1.Columns["Сomment"].HeaderText = "Комментарий";
				...
				dataGridView1.Columns["Oxide_Correction_Mat"].DefaultCellStyle.BackColor = Color.PeachPuff;
				dataGridView1.Columns["Oxide_Correction_Score"].DefaultCellStyle.BackColor = Color.PeachPuff;
		...
		private void Oxide_Oxidir_Technolog_Load(object sender, EventArgs e)
		{
			LoadData();
		...
		private void NotClickOnTable()
		{
			...
			{
				if (column.Name != "Oxide_Correction_Score" && column.Name != "Oxide_Correction_Mat" && column.Name != "Сomment") // предполагается, что "ID" - это столбец, который можно редактировать
				{
		...
		private void dataGridView1_CellEndEdit(object sender, DataGridViewCellEventArgs e)
		{
			...
			using (SqlConnection connection = new SqlConnection(WC.ConnectionString))
			{
				...
				var Oxide_H2SO4 = dataGridView1.Rows[e.RowIndex].Cells["Oxide_H2SO4"].Value;
				var Oxide_H2O2 = dataGridView1.Rows[e.RowIndex].Cells["Oxide_H2O2"].Value;
				var Oxide_Cu2 = dataGridView1.Rows[e.RowIndex].Cells["Oxide_Cu2"].Value;
				var Oxide_podtrav = dataGridView1.Rows[e.RowIndex].Cells["Oxide_podtrav"].Value;
				var Oxide_BROX1100 = dataGridView1.Rows[e.RowIndex].Cells["Oxide_BROX1100"].Value;
				var Oxide_Correction_Mat = dataGridView1.Rows[e.RowIndex].Cells["Oxide_Correction_Mat"].Value;
				var Oxide_Correction_Score = dataGridView1.Rows[e.RowIndex].Cells["Oxide_Correction_Score"].Value;
				...
				string updateQuery = @"
        UPDATE Oxide_Oxidir SET
            [Date_tech] = @Date_tech,
            [Oxide_H2SO4] = @Oxide_H2SO4
      ,[Oxide_H2O2] = @Oxide_H2O2
      ,[Oxide_Cu2] = @Oxide_Cu2,
[Oxide_podtrav] = @Oxide_podtrav,
[Oxide_BROX1100] = @Oxide_BROX1100,
[Oxide_Correction_Mat] = @Oxide_Correction_Mat,
[Oxide_Correction_Score] = @Oxide_Correction_Score
			,[Сomment] = @Comment
            ,FIO_tech = @FIO_tech
        WHERE ID = @Id";
				...
				using (SqlCommand command = new SqlCommand(updateQuery, connection))
				{
					...
					command.Parameters.AddWithValue("@Oxide_H2SO4", Oxide_H2SO4);
					command.Parameters.AddWithValue("@Oxide_H2O2", Oxide_H2O2);
					command.Parameters.AddWithValue("@Oxide_Cu2", Oxide_Cu2);
					command.Parameters.AddWithValue("@Oxide_podtrav", Oxide_podtrav);
					command.Parameters.AddWithValue("@Oxide_BROX1100", Oxide_BROX1100);
					command.Parameters.AddWithValue("@Oxide_Correction_Mat", Oxide_Correction_Mat);
					command.Parameters.AddWithValue("@Oxide_Correction_Score", Oxide_Correction_Score);
		...
		private void LoadDataDateTimePicker(DateTime date)
		{
			using (SqlConnection connection = new SqlConnection(WC.ConnectionString))
			{
				...
				string query = "SELECT * FROM Oxide_Oxidir WHERE CAST(Date_Create AS DATE) = @SelectedDate ORDER BY Date_Create DESC";
				using (SqlCommand command = new SqlCommand(query, connection))
				{
		...
		private void LoadDataByDateRange(DateTime startDate, DateTime endDate)
		{
			...
			{
				connection.Open();
				string query = "SELECT * FROM Oxide_Oxidir WHERE CAST(Date_Create AS DATE) BETWEEN @StartDate AND @EndDate ORDER BY Date_Create DESC";
				using (SqlCommand command = new SqlCommand(query, connection))
				{
			...
			if (e.RowIndex >= 0 && e.ColumnIndex == 0)
			{
				...
				if (idValue != null)
				{
					if (int.TryParse(idValue.ToString(), out WC.id_click))
					{
						...
						string connectionString = WC.ConnectionString;
						string query = @"UPDATE Oxide_Oxidir SET
                                 [Date_tech] = @Date_tech,
                                 [FIO_tech] = @FIO_tech
                                 WHERE ID = @Id";
		...
		private void HighlightEmptyCells(DataGridView dataGridView)
		{
			...
			int dateTechColumnIndex = 11; // Индекс столбца Date_tech (измените по необходимости)
		...
		private void Add_Row()
		{
			using (SqlConnection connection = new SqlConnection(WC.ConnectionString))
			{
				...
				string sqlInsert = "INSERT INTO Oxide_Oxidir ([Date_Create],[FIO_Lab]) " +
		...
		private void UpdateRow()
		{
			...
			using (SqlConnection connection = new SqlConnection(WC.ConnectionString))
			{
				...
				var Oxide_H2SO4 = dataGridView1.Rows[rowIndex].Cells["Oxide_H2SO4"].Value;
				var Oxide_H2O2 = dataGridView1.Rows[rowIndex].Cells["Oxide_H2O2"].Value;
				var Oxide_Cu2 = dataGridView1.Rows[rowIndex].Cells["Oxide_Cu2"].Value;
				var Oxide_podtrav = dataGridView1.Rows[rowIndex].Cells["Oxide_podtrav"].Value;
				var Oxide_BROX1100 = dataGridView1.Rows[rowIndex].Cells["Oxide_BROX1100"].Value;
				var Oxide_Correction_Mat = dataGridView1.Rows[rowIndex].Cells["Oxide_Correction_Mat"].Value;
				var Oxide_Correction_Score = dataGridView1.Rows[rowIndex].Cells["Oxide_Correction_Score"].Value;
				//var Comment = dataGridView1.Rows[e.RowIndex].Cells["Сomment"].Value;
