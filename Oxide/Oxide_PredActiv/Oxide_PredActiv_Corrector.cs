...
{
	public partial class Oxide_PredActiv_Corrector : Form
	{
		public Oxide_PredActiv_Corrector()
		{
			InitializeComponent();
		...
		private void LoadData()
		{
			...
			{
				connection.Open();
				dataAdapter = new SqlDataAdapter("SELECT ID, Oxide_Correction_Mat, Oxide_Correction_Score, FIO_tech, Date_tech, FIO_Corr, Date_Corr, Сompleted,[Start_corr],[Date_start_corr],[FIO_start_corr],[Сomment] FROM Oxide_PredActiv ORDER BY Date_Create DESC", connection);
...
HighlightEmptyCells(dataGridView1, columnsToCheck);
		...
		private void Oxide_PredActiv_Corrector_Load(object sender, EventArgs e)
		{
			LoadData();
		...
		private void LoadDataDateTimePicker(DateTime date)
		{
			using (SqlConnection connection = new SqlConnection(WC.ConnectionString))
			{
				...
				string query = "SELECT ID, Oxide_Correction_Mat, Oxide_Correction_Score, FIO_tech, Date_tech, FIO_Corr, Date_Corr, Сompleted,[Start_corr],[Date_start_corr],[FIO_start_corr],[Сomment] FROM Oxide_PredActiv WHERE CAST(Date_Create AS DATE) = @SelectedDate ORDER BY Date_Create DESC";
		...
		private void HighlightEmptyCells(DataGridView dataGridView, int[] columns)
{
	foreach (DataGridViewRow row in dataGridView.Rows)
	{
		...
		private void LoadDataByDateRange(DateTime startDate, DateTime endDate)
		{
			...
			{
				connection.Open();
				string query = "SELECT ID, Oxide_Correction_Mat, Oxide_Correction_Score, FIO_tech, Date_tech, FIO_Corr, Date_Corr, Сompleted,[Start_corr],[Date_start_corr],[FIO_start_corr],[Сomment] FROM Oxide_PredActiv WHERE CAST(Date_Create AS DATE) BETWEEN @StartDate AND @EndDate ORDER BY Date_Create DESC";
				using (SqlCommand command = new SqlCommand(query, connection))
				{
