...
namespace Rez_Lab
{
	public partial class Start_Oxide_correcter : Form
	{
		...
		private void Start_Oxide_correcter_Load(object sender, EventArgs e)
		{
...
//		}
		...
		{
			this.Hide();
			Oxide_ShelOch_Corrector tmc = new Oxide_ShelOch_Corrector();
			tmc.ShowDialog();
			this.Show();
		}
		...
		{
			this.Hide();
			Oxide_PredActiv_Corrector tmc = new Oxide_PredActiv_Corrector();
			tmc.ShowDialog();
			this.Show();
		}
		...
		{
			this.Hide();
			Oxide_Oxidir_Corrector tmc = new Oxide_Oxidir_Corrector();
			tmc.ShowDialog();
			this.Show();
		}
		...
		private void Load_Oxide_PredActiv()
		{
			string query = @"
         SELECT COUNT(*) 
FROM Oxide_PredActiv
WHERE ([Сompleted] IS NULL OR [Сompleted] = '') 
AND (
    ([Oxide_Correction_Mat] IS NOT NULL AND [Oxide_Correction_Mat] <> '') OR ([Сomment] IS NOT NULL AND [Сomment] <> '') OR 
    ([Oxide_Correction_Score] IS NOT NULL AND [Oxide_Correction_Score] <> ''));";
		...
		private void Load_Oxide_Oxidir()
		{
			string query = @"
         SELECT COUNT(*) 
FROM Oxide_Oxidir
...
AND (
    ...
    ([Oxide_Correction_Score] IS NOT NULL AND [Oxide_Correction_Score] <> ''));";
		...
		private void Load_Oxide_ShelOch()
		{
			string query = @"
         SELECT COUNT(*) 
FROM Oxide_ShelOch
...
AND (
    ...
    ([Oxide_Correction_Score] IS NOT NULL AND [Oxide_Correction_Score] <> ''));";
		...
		private DataSet GetAllDataFromDatabase()
		{
			...
			string query = @"
			SELECT 
                [Oxide_Correction_Mat],
                [Oxide_Correction_Score],
                [Сomment]
            FROM 
                Oxide_ShelOch
            WHERE 
                ...
                AND (
                    ...
                    OR ([Сomment] IS NOT NULL AND [Сomment] <> '') OR ([Oxide_Correction_Score] IS NOT NULL AND [Oxide_Correction_Score] <> '')
                );
