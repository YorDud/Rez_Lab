...
{
	public partial class DES_KisTrav_Technolog : Form
	{
		public DES_KisTrav_Technolog()
		{
			InitializeComponent();
...
dataGridView1.Columns["Сomment"].HeaderText = "Комментарий";
				...
				dataGridView1.Columns["Pro_Photorez_3_Correction_Mat"].DefaultCellStyle.BackColor = Color.PeachPuff;
				dataGridView1.Columns["Pro_Photorez_3_Correction_Score"].DefaultCellStyle.BackColor = Color.PeachPuff;
		...
		private void DES_KisTrav_Technolog_Load(object sender, EventArgs e)
		{
			LoadData();
		...
		private void NotClickOnTable()
		{
			...
			{
				if (column.Name != "Pro_Photorez_3_Correction_Score" && column.Name != "Pro_Photorez_3_Correction_Mat" && column.Name != "Сomment") // предполагается, что "ID" - это столбец, который можно редактировать
				{
...
[Pro_Photorez_3_Correction_Score] = @Pro_Photorez_3_Correction_Score
			...
			if (e.RowIndex >= 0 && e.ColumnIndex == 0)
			{
				...
				if (idValue != null)
				{
					if (int.TryParse(idValue.ToString(), out WC.id_click))
					{
						...
						string connectionString = WC.ConnectionString;
						string query = @"UPDATE DES_KisTrav SET
                                 [Date_tech] = @Date_tech,
                                 [FIO_tech] = @FIO_tech
                                 WHERE ID = @Id";
		...
		private void UpdateRow()
		{
			...
			using (SqlConnection connection = new SqlConnection(WC.ConnectionString))
			{
				...
				var Pro_Photorez_3_Correction_Mat = dataGridView1.Rows[rowIndex].Cells["Pro_Photorez_3_Correction_Mat"].Value;
				var Pro_Photorez_3_Correction_Score = dataGridView1.Rows[rowIndex].Cells["Pro_Photorez_3_Correction_Score"].Value;
				//var Comment = dataGridView1.Rows[e.RowIndex].Cells["Сomment"].Value;
				...
				string updateQuery = @"
            UPDATE DES_KisTrav
   SET [Pro_Photorez_3_Correction_Mat] = @Pro_Photorez_3_Correction_Mat,
[Pro_Photorez_3_Correction_Score] = @Pro_Photorez_3_Correction_Score
