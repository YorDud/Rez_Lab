...
{
	public partial class DES_Ult1_Corrector : Form
	{
		public DES_Ult1_Corrector()
		{
			InitializeComponent();
		...
		private void LoadData()
		{
			...
			{
				connection.Open();
				dataAdapter = new SqlDataAdapter("SELECT ID, Pro_Photorez_2_Correction_Mat, Pro_Photorez_2_Correction_Score, FIO_tech, Date_tech, FIO_Corr, Date_Corr, Сompleted,[Start_corr],[Date_start_corr],[FIO_start_corr],[Сomment] FROM DES_Ult1 ORDER BY Date_Create DESC", connection);
...
HighlightEmptyCells(dataGridView1, columnsToCheck);
		...
		private void DES_Ult1_Corrector_Load(object sender, EventArgs e)
		{
			LoadData();
		...
		private void LoadDataDateTimePicker(DateTime date)
		{
			using (SqlConnection connection = new SqlConnection(WC.ConnectionString))
			{
				...
				string query = "SELECT ID, Pro_Photorez_2_Correction_Mat, Pro_Photorez_2_Correction_Score, FIO_tech, Date_tech, FIO_Corr, Date_Corr, Сompleted,[Start_corr],[Date_start_corr],[FIO_start_corr],[Сomment] FROM DES_Ult1 WHERE CAST(Date_Create AS DATE) = @SelectedDate ORDER BY Date_Create DESC";
		...
		private void HighlightEmptyCells(DataGridView dataGridView, int[] columns)
{
	foreach (DataGridViewRow row in dataGridView.Rows)
	{
		...
		private void LoadDataByDateRange(DateTime startDate, DateTime endDate)
		{
			...
			{
				connection.Open();
				string query = "SELECT ID, Pro_Photorez_2_Correction_Mat, Pro_Photorez_2_Correction_Score, FIO_tech, Date_tech, FIO_Corr, Date_Corr, Сompleted,[Start_corr],[Date_start_corr],[FIO_start_corr],[Сomment] FROM DES_Ult1 WHERE CAST(Date_Create AS DATE) BETWEEN @StartDate AND @EndDate ORDER BY Date_Create DESC";
				using (SqlCommand command = new SqlCommand(query, connection))
				{
		...
		private void dataGridView1_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
		{
			...
			if (e.RowIndex >= 0 && columnsToCheck.Contains(e.ColumnIndex))
			{
				...
				var Correction_Mat_Value = selectedRow.Cells["Pro_Photorez_2_Correction_Mat"].Value;
				var Correction_Score_Value = selectedRow.Cells["Pro_Photorez_2_Correction_Score"].Value;
