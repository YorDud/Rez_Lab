...
{
	public partial class DES_ProvMod_Technolog : Form
	{
		public DES_ProvMod_Technolog()
		{
			InitializeComponent();
		...
		private void LoadData()
		{
			...
			{
				connection.Open();
				dataAdapter = new SqlDataAdapter("SELECT * FROM DES_ProvMod ORDER BY Date_Create DESC", connection);
...
dataGridView1.Columns["Сomment"].HeaderText = "Комментарий";
				...
				dataGridView1.Columns["Pro_Photorez_1_Correction_Mat"].DefaultCellStyle.BackColor = Color.PeachPuff;
				dataGridView1.Columns["Pro_Photorez_1_Correction_Score"].DefaultCellStyle.BackColor = Color.PeachPuff;
		...
		private void DES_ProvMod_Technolog_Load(object sender, EventArgs e)
		{
			LoadData();
		...
		private void NotClickOnTable()
		{
			...
			{
				if (column.Name != "Pro_Photorez_1_Correction_Score" && column.Name != "Pro_Photorez_1_Correction_Mat" && column.Name != "Сomment") // предполагается, что "ID" - это столбец, который можно редактировать
				{
		...
		private void dataGridView1_CellEndEdit(object sender, DataGridViewCellEventArgs e)
		{
			...
			using (SqlConnection connection = new SqlConnection(WC.ConnectionString))
			{
				...
				var Pro_Photorez_1_Correction_Mat = dataGridView1.Rows[e.RowIndex].Cells["Pro_Photorez_1_Correction_Mat"].Value;
				var Pro_Photorez_1_Correction_Score = dataGridView1.Rows[e.RowIndex].Cells["Pro_Photorez_1_Correction_Score"].Value;
				...
				string updateQuery = @"
        UPDATE DES_ProvMod SET
            [Date_tech] = @Date_tech,
            [Pro_Photorez_1_Correction_Mat] = @Pro_Photorez_1_Correction_Mat
			,[Pro_Photorez_1_Correction_Score] = @Pro_Photorez_1_Correction_Score
			,[Сomment] = @Comment
            ,FIO_tech = @FIO_tech
        WHERE ID = @Id";
				...
				using (SqlCommand command = new SqlCommand(updateQuery, connection))
				{
					...
					command.Parameters.AddWithValue("@Pro_Photorez_1_Correction_Mat", Pro_Photorez_1_Correction_Mat);
					command.Parameters.AddWithValue("@Pro_Photorez_1_Correction_Score", Pro_Photorez_1_Correction_Score);
		...
		private void LoadDataDateTimePicker(DateTime date)
		{
			using (SqlConnection connection = new SqlConnection(WC.ConnectionString))
			{
				...
				string query = "SELECT * FROM DES_ProvMod WHERE CAST(Date_Create AS DATE) = @SelectedDate ORDER BY Date_Create DESC";
				using (SqlCommand command = new SqlCommand(query, connection))
				{
		...
		private void LoadDataByDateRange(DateTime startDate, DateTime endDate)
		{
			...
			{
				connection.Open();
				string query = "SELECT * FROM DES_ProvMod WHERE CAST(Date_Create AS DATE) BETWEEN @StartDate AND @EndDate ORDER BY Date_Create DESC";
				using (SqlCommand command = new SqlCommand(query, connection))
				{
			...
			if (e.RowIndex >= 0 && e.ColumnIndex == 0)
			{
				...
				if (idValue != null)
				{
					if (int.TryParse(idValue.ToString(), out WC.id_click))
					{
						...
						string connectionString = WC.ConnectionString;
						string query = @"UPDATE DES_ProvMod SET
                                 [Date_tech] = @Date_tech,
                                 [FIO_tech] = @FIO_tech
                                 WHERE ID = @Id";
		...
		private void Add_Row()
		{
			using (SqlConnection connection = new SqlConnection(WC.ConnectionString))
			{
				...
				string sqlInsert = "INSERT INTO DES_ProvMod ([Date_Create],[FIO_Lab]) " +
		...
		private void UpdateRow()
		{
			...
			using (SqlConnection connection = new SqlConnection(WC.ConnectionString))
			{
				...
				var Pro_Photorez_1_Correction_Mat = dataGridView1.Rows[rowIndex].Cells["Pro_Photorez_1_Correction_Mat"].Value;
				var Pro_Photorez_1_Correction_Score = dataGridView1.Rows[rowIndex].Cells["Pro_Photorez_1_Correction_Score"].Value;
				//var Comment = dataGridView1.Rows[e.RowIndex].Cells["Сomment"].Value;
				...
				string updateQuery = @"
            UPDATE DES_ProvMod
   SET [Pro_Photorez_1_Correction_Mat] = @Pro_Photorez_1_Correction_Mat
			,[Pro_Photorez_1_Correction_Score] = @Pro_Photorez_1_Correction_Score
