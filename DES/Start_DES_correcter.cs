...
using Rez_Lab.DES_Snatie_Photorez;
...
namespace Rez_Lab
{
	public partial class Start_DES_correcter : Form
	{
		...
		private void Start_DES_correcter_Load(object sender, EventArgs e)
		{
...
//    ([Pro_Photorez_3_Correction] IS NOT NULL AND [Pro_Photorez_3_Correction] <> '') OR 
//    ([Pro_Photorez_4_Correction] IS NOT NULL AND [Pro_Photorez_4_Correction] <> '')
//);
//";
...
//		}
		...
		{
			//this.Hide();
			//DES_Corrector tmc = new DES_Corrector();
			//tmc.ShowDialog();
			//this.Show();
		}
...
//		}
		...
		{
			this.Hide();
			DES_ProvMod_Corrector tmc = new DES_ProvMod_Corrector();
			tmc.ShowDialog();
			this.Show();
		}
		...
		{
			this.Hide();
			DES_Ult1_Corrector tmc = new DES_Ult1_Corrector();
			tmc.ShowDialog();
			this.Show();
		}
		...
		{
			this.Hide();
			DES_KisTrav_Corrector tmc = new DES_KisTrav_Corrector();
			tmc.ShowDialog();
			this.Show();
		}
...
WHERE ([Сompleted] IS NULL OR [Сompleted] = '') 
AND (
    ([Pro_Photorez_1_Correction_Mat] IS NOT NULL AND [Pro_Photorez_1_Correction_Mat] <> '') OR ([Сomment] IS NOT NULL AND [Сomment] <> '') OR 
...
WHERE ([Сompleted] IS NULL OR [Сompleted] = '') 
AND (
    ([Pro_Photorez_2_Correction_Mat] IS NOT NULL AND [Pro_Photorez_2_Correction_Mat] <> '') OR ([Сomment] IS NOT NULL AND [Сomment] <> '') OR 
...
WHERE ([Сompleted] IS NULL OR [Сompleted] = '') 
AND (
    ([Pro_Photorez_3_Correction_Mat] IS NOT NULL AND [Pro_Photorez_3_Correction_Mat] <> '') OR ([Сomment] IS NOT NULL AND [Сomment] <> '') OR 
    ([Pro_Photorez_3_Correction_Score] IS NOT NULL AND [Pro_Photorez_3_Correction_Score] <> ''));";
		...
		private readonly Font dateFont = new Font("Arial", 14, FontStyle.Bold);
	...
	"!!! Снятие фоторезиста !!!"
		...
		private readonly Font headerFont = new Font("Arial", 12, FontStyle.Bold);
		private readonly Font columnFont = new Font("Arial", 8, FontStyle.Bold);
		private readonly Font dataFont = new Font("Arial", 12, FontStyle.Bold);
		...
		private DataSet GetAllDataFromDatabase()
		{
			try
			{
				...
				string query = @"
            SELECT 
                [Pro_Photorez_1_Correction_Mat] AS Correction_Mat,
                [Pro_Photorez_1_Correction_Score] AS Correction_Score,
                [Сomment],
                NULL AS Sn_Photorez_1_Correction,
                NULL AS Sn_Photorez_2_Correction,
                NULL AS Sn_Photorez_3_Correction,
                'DES_ProvMod' AS SourceTable
            FROM DES_ProvMod
            WHERE ([Сompleted] IS NULL OR [Сompleted] = '') 
            AND (
            ...
            UNION ALL
            SELECT 
                [Pro_Photorez_2_Correction_Mat],
                [Pro_Photorez_2_Correction_Score],
                [Сomment],
                NULL,
                NULL,
                NULL,
                'DES_Ult1' AS SourceTable
            FROM DES_Ult1
            WHERE ([Сompleted] IS NULL OR [Сompleted] = '') 
            AND (
            ...
            UNION ALL
            SELECT 
                [Pro_Photorez_3_Correction_Mat],
                [Pro_Photorez_3_Correction_Score],
                [Сomment],
                NULL,
                NULL,
                NULL,
                'DES_KisTrav' AS SourceTable
            FROM DES_KisTrav
            WHERE ([Сompleted] IS NULL OR [Сompleted] = '') 
            AND (
            ...
            UNION ALL
            SELECT 
                NULL AS Correction_Mat,
                NULL AS Correction_Score,
                [Сomment],
                [Sn_Photorez_1_Correction],
                [Sn_Photorez_2_Correction],
                [Sn_Photorez_3_Correction],
                'DES_Snatie_Photorez' AS SourceTable
            FROM DES_Snatie_Photorez
            WHERE ([Сompleted] IS NULL OR [Сompleted] = '') 
            AND (
				...
				using (SqlConnection connection = new SqlConnection(WC.ConnectionString))
				{
					using (SqlDataAdapter adapter = new SqlDataAdapter(query, connection))
					{
						connection.Open();
						adapter.Fill(ds);
						ds.Tables[0].TableName = "CombinedData";
					}
				}
				...
				if (ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
				{
					var groupedData = ds.Tables[0].AsEnumerable()
						.GroupBy(row => row.Field<string>("SourceTable"));

					foreach (var group in groupedData)
					{
						string sourceTable = group.Key;
						int index = -1;

						if (sourceTable == "DES_ProvMod")
							index = Array.FindIndex(tableHeaders, h => h.Contains("Проявочный модуль"));
						else if (sourceTable == "DES_Ult1")
							index = Array.FindIndex(tableHeaders, h => h.Contains("Ультрафильтрация"));
						else if (sourceTable == "DES_KisTrav")
							index = Array.FindIndex(tableHeaders, h => h.Contains("Кислое травление"));
						else if (sourceTable == "DES_Snatie_Photorez")
							index = Array.FindIndex(tableHeaders, h => h.Contains("Снятие фоторезиста"));

						filteredTableHeaders.Add(index >= 0 ? tableHeaders[index] : $"Таблица {sourceTable}");
					}
				}
		...
		private void PrintDocument_PrintPage(object sender, PrintPageEventArgs e)
		{
			...
			var groupedData = table.AsEnumerable()
				.GroupBy(row => row.Field<string>("SourceTable"))
				.Skip(currentTableIndex);

			foreach (var group in groupedData)
			{
				if (currentTableIndex >= filteredTableHeaders.Count) break;

				string header = filteredTableHeaders[currentTableIndex];
				e.Graphics.DrawString(header, headerFont, Brushes.Black, leftMargin, yPos, stringFormat);
				yPos += lineHeight + 5;

				// Определяем заголовки и ширины колонок в зависимости от таблицы
				string[] columnNames;
				float[] currentColumnWidths;
				bool isSnatiePhotorez = group.Key == "DES_Snatie_Photorez";

				if (isSnatiePhotorez)
				{
					columnNames = new[] { "Корр. 1", "Корр. 2", "Корр. 3", "Комментарии" };
					currentColumnWidths = new float[] { 150, 150, 150, 250 }; // 4 колонки
				}
				else
				{
					columnNames = new[] { "Корр. Материал", "Корр. Количество", "Комментарии" };
					currentColumnWidths = columnWidths; // Оригинальные 3 колонки
				}

				float currentLeft = leftMargin;
				for (int i = 0; i < columnNames.Length; i++)
				{
					RectangleF headerRect = new RectangleF(currentLeft, yPos, currentColumnWidths[i], lineHeight);
					e.Graphics.DrawString(columnNames[i], columnFont, Brushes.Black, headerRect, stringFormat);
					currentLeft += currentColumnWidths[i];
				}
				yPos += lineHeight;

				var rows = group.Skip(currentRowIndex);
				foreach (DataRow row in rows)
				{
					currentLeft = leftMargin;
					float maxHeight = 0;
					SizeF[] sizes = new SizeF[columnNames.Length];
					...
					for (int i = 0; i < columnNames.Length; i++)
					{
						...
						if (isSnatiePhotorez)
						{
							if (i == 0) text = row["Sn_Photorez_1_Correction"]?.ToString() ?? "";
							else if (i == 1) text = row["Sn_Photorez_2_Correction"]?.ToString() ?? "";
							else if (i == 2) text = row["Sn_Photorez_3_Correction"]?.ToString() ?? "";
							else if (i == 3) text = row["Сomment"]?.ToString() ?? "";
							else text = "";
						}
						else
						{
						...
						sizes[i] = e.Graphics.MeasureString(text, dataFont, (int)currentColumnWidths[i], stringFormat);
					...
					for (int i = 0; i < columnNames.Length; i++)
					{
						...
						RectangleF cellRect = new RectangleF(currentLeft, yPos, currentColumnWidths[i], maxHeight);
		...
		//    NULL AS Correction_Score,
		//    [Сomment],
		//    'Snatie_Photorez_2' AS SourceTable
		//FROM 
		//    Snatie_Photorez_2
		//WHERE 
		//    ([Сompleted] IS NULL OR [Сompleted] = '') 
		...
		//			string title = "Проявление Фоторезиста";
		//			SizeF titleSize = e.Graphics.MeasureString(title, titleFont);
		//			float titleX = leftMargin + (e.MarginBounds.Width - titleSize.Width) / 2;
		//			e.Graphics.DrawString(title, titleFont, Brushes.Black, titleX, yPos);
		//			yPos += titleSize.Height + 20;
		...
		{
			this.Hide();
			DES_Snatie_Photorez_Corrector tmc = new DES_Snatie_Photorez_Corrector();
			tmc.ShowDialog();
			this.Show();
		}
		...
		private void Load_DES_Snatie_Photorez()
		{
			string query = @"
        SELECT COUNT(*) 
FROM DES_Snatie_Photorez 
...
";
		...
		//WHERE ([Сompleted] IS NULL OR [Сompleted] = '') 
		//AND (
		//    ([Pro_Photorez_4_Correction_Mat] IS NOT NULL AND [Pro_Photorez_4_Correction_Mat] <> '') OR 
		//    ([Pro_Photorez_4_Correction_Score] IS NOT NULL AND [Pro_Photorez_4_Correction_Score] <> ''));";
