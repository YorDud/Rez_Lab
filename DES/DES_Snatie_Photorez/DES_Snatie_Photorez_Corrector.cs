...
{
	public partial class DES_Snatie_Photorez_Corrector : Form
	{
		public DES_Snatie_Photorez_Corrector()
		{
			InitializeComponent();
		...
		private void LoadData()
		{
			...
			{
				connection.Open();
				dataAdapter = new SqlDataAdapter("SELECT ID, Sn_Photorez_1_Correction, Sn_Photorez_2_Correction, Sn_Photorez_3_Correction, FIO_tech, Date_tech, FIO_Corr, Date_Corr, Сompleted,[Start_corr],[Date_start_corr],[FIO_start_corr],[Сomment] FROM DES_Snatie_Photorez ORDER BY Date_Create DESC", connection);
...
HighlightEmptyCells(dataGridView1, columnsToCheck);
		...
		private void DES_Snatie_Photorez_Corrector_Load(object sender, EventArgs e)
		{
			LoadData();
		...
		private void LoadDataDateTimePicker(DateTime date)
		{
			using (SqlConnection connection = new SqlConnection(WC.ConnectionString))
			{
				...
				string query = "SELECT ID, Sn_Photorez_1_Correction, Sn_Photorez_2_Correction, Sn_Photorez_3_Correction, FIO_tech, Date_tech, FIO_Corr, Date_Corr, Сompleted,[Start_corr],[Date_start_corr],[FIO_start_corr],[Сomment] FROM DES_Snatie_Photorez WHERE CAST(Date_Create AS DATE) = @SelectedDate ORDER BY Date_Create DESC";
			...
			if (e.RowIndex >= 0 && columnsToCheck.Contains(e.ColumnIndex))
			{
				...
				if (idValue != null)
				{
					if (int.TryParse(idValue.ToString(), out int idClick))
					{
						...
						string query = @"UPDATE DES_Snatie_Photorez SET 
                                 [Date_Corr] = @Date_Corr,
                                 [Сompleted] = @Сompleted,
                                 [FIO_Corr] = @FIO_Corr
                                 WHERE ID = @Id";
