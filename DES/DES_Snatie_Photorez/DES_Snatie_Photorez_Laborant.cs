...
{
	public partial class DES_Snatie_Photorez_Laborant : Form
	{
		public DES_Snatie_Photorez_Laborant()
		{
			InitializeComponent();
		...
		private void LoadData()
		{
			...
			{
				connection.Open();
				dataAdapter = new SqlDataAdapter("SELECT * FROM DES_Snatie_Photorez ORDER BY Date_Create DESC", connection);
...
dataGridView1.Columns["Сomment"].HeaderText = "Комментарий";
				...
				dataGridView1.Columns["Sn_Photorez_1_MS358A"].DefaultCellStyle.BackColor = Color.LightBlue;
				dataGridView1.Columns["Sn_Photorez_2_MS358A"].DefaultCellStyle.BackColor = Color.LightBlue;
				dataGridView1.Columns["Sn_Photorez_3_MS358A"].DefaultCellStyle.BackColor = Color.LightBlue;
		...
		private void DES_Snatie_Photorez_Laborant_Load(object sender, EventArgs e)
		{
			LoadData();
		...
		private void LoadDataDateTimePicker(DateTime date)
		{
			using (SqlConnection connection = new SqlConnection(WC.ConnectionString))
			{
				...
				string query = "SELECT * FROM DES_Snatie_Photorez WHERE CAST(Date_Create AS DATE) = @SelectedDate ORDER BY Date_Create DESC";
		...
		private void LoadDataByDateRange(DateTime startDate, DateTime endDate)
		{
			...
			{
				connection.Open();
				string query = "SELECT * FROM DES_Snatie_Photorez WHERE CAST(Date_Create AS DATE) BETWEEN @StartDate AND @EndDate ORDER BY Date_Create DESC";
				using (SqlCommand command = new SqlCommand(query, connection))
				{
		...
		private void Add_Row()
		{
			using (SqlConnection connection = new SqlConnection(WC.ConnectionString))
			{
				...
				string sqlInsert = "INSERT INTO DES_Snatie_Photorez ([Date_Create],[FIO_Lab]) " +
		...
		private void UpdateRow()
		{
			...
			using (SqlConnection connection = new SqlConnection(WC.ConnectionString))
			{
				...
				var Sn_Photorez_1_MS358A = dataGridView1.Rows[rowIndex].Cells["Sn_Photorez_1_MS358A"].Value;

				var Sn_Photorez_2_MS358A = dataGridView1.Rows[rowIndex].Cells["Sn_Photorez_2_MS358A"].Value;

				var Sn_Photorez_3_MS358A = dataGridView1.Rows[rowIndex].Cells["Sn_Photorez_3_MS358A"].Value;
...
[Sn_Photorez_3_MS358A] = @Sn_Photorez_3_MS358A
	...
	WHERE ID = @ID";
		...
		private void NotClickOnTable()
		{
			...
			{
				if (column.Name != "Sn_Photorez_1_MS358A" && column.Name != "Sn_Photorez_2_MS358A" &&
					column.Name != "Sn_Photorez_3_MS358A")
				{
